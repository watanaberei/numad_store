...
const testConnection = async () => {
  try {
    ...
    console.log(`Total stores in database: ${storedCount}\n`);
...
const syncAllStores = async () => {
  try {
    ...
    const results = await Promise.all(storesToSync.map(async (store) => {
      try {
        ...
        console.error(`✗ Error syncing ${store.headline?.text}: ${error.message}`);
        return null;
      }
    }));
    ...
    console.log(`\n=== Sync Complete ===`);
    console.log(`Successfully synced ${successCount}/${storesToSync.length} stores\n`);
