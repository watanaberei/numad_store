...
app.get('/api/stores', async (req, res) => {
  try {
    ...
    console.error('[Server.js] Store listing error:', error);
    res.status(500).json({ 
      success: false,
...
app.post('/api/store/checkin', authenticationToken, async (req, res) => {
  try {
    ...
    console.log(`[Server] Check-in request: ${action} for store ${storeId} by user ${userId}`);
...
app.post('/api/store/impression', authenticationToken, async (req, res) => {
  try {
    ...
    console.log(`[Server] Impression request: ${action} for store ${storeId} section ${section} by user ${userId}`);
    ...
    console.error('[Server] Impression error:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message 
    });
...
app.get('/api/store/checkin/status', authenticationToken, async (req, res) => {
  try {
    ...
    console.error('[Server] Check-in status error:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message 
    });
...
app.get('/api/user/impressions/:storeId', authenticationToken, async (req, res) => {
  try {
    ...
    console.error('[Server] User impressions error:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message 
    });
...
app.post('/api/blog/sync/:slug', async (req, res) => {
  try {
    ...
    ///////////////////////// START USER BLOG POSTS UPDATE /////////////////////////
    // Update user's blogPosts array if author information is available
    if (blogData.author && blogData.author.username) {
      try {
        console.log(`[Server] Updating user blog posts for author: ${blogData.author.username}`);
        
        const user = await UserModel.findOne({ username: blogData.author.username });
        
        if (user) {
          // Check if blog already exists in user's blogPosts
          const existingIndex = user.blogPosts.findIndex(
            p => p.blogId?.toString() === result._id.toString()
          );
          ...
          const blogPostData = {
            ...
            title: result.title,
            slug: result.slug,
            category: result.category?.category || 'dine',
            snippet: result.snippet?.text || '',
            thumbnail: result.media?.thumbnail || result.media?.hero || '',
            status: result.status,
            publishedAt: result.publishedAt,
            addedAt: existingIndex >= 0 ? user.blogPosts[existingIndex].addedAt : new Date()
          };
          
          if (existingIndex >= 0) {
            // Update existing blog post
            user.blogPosts[existingIndex] = blogPostData;
            console.log(`[Server] Updated existing blog post in user's array`);
          } else {
            // Add new blog post
            user.blogPosts.push(blogPostData);
            console.log(`[Server] Added new blog post to user's array`);
          }
          
          // Also update blogsCreated if it's a new blog
          const createdIndex = user.blogsCreated.findIndex(
            b => b.blogId?.toString() === result._id.toString()
          );
          ...
          await user.save();
          console.log(`[Server] User blog posts updated successfully for: ${user.username}`);
        } else {
          console.log(`[Server] User not found for username: ${blogData.author.username}`);
        }
      } catch (userError) {
        console.error(`[Server] Error updating user blog posts:`, userError);
        // Don't fail the whole request if user update fails
      }
    }
    ///////////////////////// END USER BLOG POSTS UPDATE /////////////////////////
  ...
  // Add endpoint to sync all blogs for a user
  app.post('/api/user/:username/sync-blogs', authenticationToken, async (req, res) => {
    try {
      ...
      console.log(`[Server] Found ${blogs.length} blogs for user: ${username}`);
      
      // Clear existing blogPosts and rebuild
      user.blogPosts = [];
      user.blogsCreated = [];
      ...
      console.log(`[Server] Successfully synced ${blogs.length} blogs for user: ${username}`);
      
      res.json({
        success: true,
        message: `Synced ${blogs.length} blogs`,
        blogCount: blogs.length
      });
      
    } catch (error) {
      console.error('[Server] Error syncing user blogs:', error);
      res.status(500).json({
        success: false,
  ...
  try {
    ...
    const features = blogs.map(blog => ({
      ...
      geometry: {
        type: 'Point',
        coordinates: [
          blog.location?.geolocation?.lon || -74.006,
...
const initServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log(`  - GET    /api/blog/:slug               - Get single blog`);
      console.log(`  - POST   /api/blog/sync/:slug          - Sync/create blog`);
      console.log(`  - GET    /api/blog                     - Get all blogs`);
      console.log(`  - GET    /api/geojson/blogs            - Get blogs for map`);
      
      console.log(`Available user endpoints:`);
      console.log(`  - GET    /api/profile                  - Get current user profile (auth)`);
      console.log(`  - POST   /api/settings                 - Update user profile (auth)`);
      console.log(`  - GET    /api/user/:username           - Get public user profile`);
      console.log(`  - GET    /api/@:username               - Get public user profile`);
      console.log(`  - GET    /api/@:username/activity      - Get public user activity`);
      console.log(`  - GET    /api/user                     - Get current user data (auth)`);
      console.log(`  - GET    /api/user/store               - Get user store data (auth)`);
      console.log(`  - GET    /api/search                   - Search users`);
      console.log(`  - POST   /api/check-username           - Check username availability (auth)`);
    });
...
//       console.log(`Blog API available at: http://localhost:${PORT}/api/@:username/:slug`);
//       console.log(`Store API available at: http://localhost:${PORT}/api/stores`);
//       console.log(`User API available at: http://localhost:${PORT}/api/@:username`);  // ADD THIS LINE
...
//       // ADD THESE USER ENDPOINT LOGS:
//       console.log(`Available user endpoints:`);
//       console.log(`  - GET    /api/@:username                 - Get public user profile`);
//       console.log(`  - GET    /api/@:username/activity        - Get public user activity`);
//       console.log(`  - GET    /api/@:username/places          - Get public user places`);
//       console.log(`  - GET    /api/@:username/posts           - Get public user posts`);
//       console.log(`  - GET    /api/@:username/followers       - Get user followers`);
//       console.log(`  - GET    /api/@:username/following       - Get user following`);
//       console.log(`  - POST   /api/@:username/follow          - Follow/unfollow user`);
//       console.log(`  - POST   /api/@:username/endorse         - Endorse user`);
//       console.log(`  - POST   /api/@:username/block           - Block/unblock user`);
//       console.log(`  - GET    /api/profile                    - Get current user profile (auth)`);
//       console.log(`  - POST   /api/profile                    - Update user profile (auth)`);
//       console.log(`  - GET    /api/api/user                   - Get current user data (auth)`);
//       console.log(`  - GET    /api/store                      - Get user store data (auth)`);
//       console.log(`  - GET    /api/search                     - Search users`);
//       console.log(`  - POST   /api/check-username             - Check username availability (auth)`);
//     });
