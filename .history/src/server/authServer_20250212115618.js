...
import { Users, Stores } from "./data/mongodb/mongodb.js";
...
app.post('/api/impression', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    let store = await Stores.findOne({ storeId });
    if (!store) {
      ...
      if (storeData && storeData.length > 0) {
        ...
        return res.status(404).json({ message: 'Stores not found in Contentful' });
      }
    }
