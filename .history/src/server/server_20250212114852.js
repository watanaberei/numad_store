...
import { Users, Stores } from "./data/mongodb/mongodb.js"; // Update this line
...
app.post('/settings', async (req, res) => {
  ...
  try {
    ...
    await user.save();
    res.json({ message: 'Users settings updated successfully' });
  } catch (error) {
...
app.get('/stores/:id?', async (req, res, next) => {
  try {
    let stores;
    if (req.params.id) {
      stores = await Stores.findById(req.params.id);
      if (!stores) {
