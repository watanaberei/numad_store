...
// Add this to authServer.js - Updated Impression Endpoint
app.post('/api/impression', authenticateToken, async (req, res) => {
  const { storeId, action } = req.body;
  const userEmail = req.user.email;
  
  console.log(`[authServer] Impression ${action} request for store ${storeId} by user ${userEmail}`);
  
  try {
    // Check if user is checked in to the store
    const user = await UserModel.findOne({ email: userEmail });
    if (!user) {
      return res.status(404).json({ success: false, message: 'User not found' });
    }
    
    // Verify user is checked in to this store
    if (user.checkedInStore !== storeId) {
      return res.status(403).json({ 
        success: false, 
        message: 'You must be checked in to this store to like or dislike it' 
      });
    }
    ...
    if (!store) {
      // Create a minimal store record without full validation
      store = new StoreModel();
      store.slug = storeId;
      store.likes = 0;
      store.dislikes = 0;
      
      // Set isNew flag to bypass schema validation for required fields
      // This is a Mongoose trick to avoid validation errors
      store.$__.isNew = false;
      
      await store.save({ validateBeforeSave: false });
    }
    
    // Initialize likes/dislikes if they don't exist
    if (typeof store.likes !== 'number') store.likes = 0;
    if (typeof store.dislikes !== 'number') store.dislikes = 0;
    
    // Handle impression action
    if (action === 'like') {
      // If user already liked, remove the like
      if (user.likedStores && user.likedStores.includes(storeId)) {
      ...
      // Otherwise add the like and remove any dislike
        ...
        // Remove from disliked stores if present
        if (user.dislikedStores && user.dislikedStores.includes(storeId)) {
          user.dislikedStores = user.dislikedStores.filter(id => id !== storeId);
          store.dislikes = Math.max(0, store.dislikes - 1);
        }
    ...
    // Save changes
    await Promise.all([user.save(), store.save({ validateBeforeSave: false })]);
    ...
    console.log(`[authServer] Impression updated: ${storeId} - Likes: ${store.likes}, Dislikes: ${store.dislikes}`);
    ...
    return res.status(500).json({ 
      success: false, 
      message: 'Server error processing impression request',
      error: error.message
    });
  }
});
