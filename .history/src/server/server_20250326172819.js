...
app.get('/api/stores/current', async (req, res) => {
  try {
    const defaultSlug = 'ca-los-angeles-county-cerritos-smoking-tiger-bread-factory';
    res.json({ slug: defaultSlug });
  } catch (error) {
...
app.get('/api/stores/:slug', async (req, res) => {
  try {
    ...
    let store = await Store.findOne({ slug: normalizedSlug });
...
const getStoreSlug = async () => {
  try {
    ...
    const storeData = await makeServerRequest(`${API_URL}/api/store/:slug`, 'POST');
