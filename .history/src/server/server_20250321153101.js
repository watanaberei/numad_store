...
app.post('/api/stores/visit/:slug', authenticateUser, async (req, res) => {
  try {
    ...
    console.log('[Store Visit] Store details:', storeDetails);
    ...
    if (storeDetails.location) {
      const yelpParams = {
        ...
        storeCity: storeDetails.location.city,
        storeState: storeDetails.location.state,
        storeLongitude: storeDetails.location.geolocation?.lon,
        storeLatitude: storeDetails.location.geolocation?.lat
      };
      yelpData = await Yelp.getStoreData(yelpParams);
      console.log('[Store Visit] Yelp data:', yelpData);
    }

    // 3. Format store data for MongoDB
    const storeDoc = {
      ...
      hours: yelpData?.hours || storeDetails.hours || [],
      ...
      location: {
        type: 'Store',
        geolocation: {
          lat: storeDetails.location?.geolocation?.lat || yelpData?.coordinates?.latitude || 0,
          lon: storeDetails.location?.geolocation?.lon || yelpData?.coordinates?.longitude || 0
        },
        address: storeDetails.location?.address || yelpData?.location?.address1 || '',
        region: storeDetails.location?.region || yelpData?.location?.city || '',
