...
// Start the server and handle dynamic store loading
const startServer = async () => {
  try {
    ...
    // 2. Start server first so it can receive requests
    const port = process.env.SERVERPORT || 6000;
    app.listen(port, () => {
      console.log(`[Init] Server running on port ${port}`);
    });

    // 3. Initialize with a default store
    const defaultSlug = 'ca_orange-county_newport_blueBottle-coffee';
    console.log('[Init] Starting with default store:', defaultSlug);
    
    const storeData = new data.StoreData();
    const storeSlug = await storeData.store(slug);
    await initializeServer(storeSlug);
