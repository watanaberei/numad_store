...
async function syncAllUserBlogs() {
    ...
    console.log('[SYNC] functionUserBlog.js:25 - Attempting database connection...');
    
    try {
        await connectDB();
        console.log('[SYNC] functionUserBlog.js:28 - Connected successfully!\n');
    } catch (error) {
        console.error('[SYNC] functionUserBlog.js:30 - Database connection failed:', error.message);
        throw new Error(`Database connection failed: ${error.message}`);
    }
    
    // Get all unique authors from blogs
    console.log('[SYNC] functionUserBlog.js:34 - Finding all blog authors...');
    try {
        const authors = await BlogModel.distinct('author.username');
        console.log(`[SYNC] functionUserBlog.js:37 - Found ${authors.length} unique authors with blogs\n`);
    } catch (error) {
        console.error('[SYNC] functionUserBlog.js:39 - Error finding authors:', error.message);
