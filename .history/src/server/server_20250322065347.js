...
// Initialize store data tracking
const storeDataset = {
  errors: [],
  visits: [],
  slugs: new Set()
...
// Get store slug from current request
const getStoreSlugFromRequest = async () => {
  try {
    // Get current request URL params
    const request = parseRequestUrl();
    console.log('[Server] Current request:', request);

    if (!request || !request.slug) {
      const error = {
        timestamp: new Date().toISOString(),
        type: 'MISSING_SLUG',
        message: 'No slug found in request URL',
        request
      };
      storeDataset.errors.push(error);
      console.error('[Server] Error:', error);
      return null;
    }

    // Track this slug visit
    storeDataset.slugs.add(request.slug);
    storeDataset.visits.push({
      timestamp: new Date().toISOString(),
      slug: request.slug
    });
    ...
    const errorData = {
      timestamp: new Date().toISOString(), 
      type: 'SLUG_FETCH_ERROR',
      message: error.message,
      error
    };
    storeDataset.errors.push(errorData);
    console.error('[Server] Error getting slug:', errorData);
    return null;
  }
};

// Initialize server with dynamic store data
const initServer = async () => {
  try {
    ...
    // Get store slug from current request
    const slug = await getStoreSlugFromRequest();
    ...
    if (slug) {
      ...
      throw new Error('No valid slug available for initialization');
    }
    ...
    storeDataset.errors.push(errorData);
    console.error('[Server] Initialization error:', errorData);
  }
};
...
// Expose dataset endpoint
app.get('/api/store-dataset', (req, res) => {
  res.json(storeDataset);
});
