...
// Get store slug from StoreScreen and log activity
const getStoreSlugFromScreen = async () => {
  try {
    const request = parseRequestUrl();
    console.log('[Server] Current request URL:', request);
    
    if (!request || !request.slug) {
      const error = new Error('No slug found in request');
      storeDataLog.errors.push({
    ...
    storeDataLog.visits.push({
      timestamp: new Date().toISOString(),
      slug: request.slug,
      path: request.path
    });

    storeDataLog.currentSlug = request.slug;
    storeDataLog.lastUpdate = new Date().toISOString();
...
// Initialize server with store tracking
const initServer = async () => {
  try {
    const slug = await getStoreSlugFromScreen();
    ...
    app.listen(port, () => {
      ...
      console.log(`Store data log:`, storeDataLog);
    });
    ...
    // Continue running server even if store init fails
    app.listen(port, () => {
      console.log(`Server running on port ${port} (Store initialization failed)`);
      console.log(`Error log:`, storeDataLog.errors);
    });
  }
};
