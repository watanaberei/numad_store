...
app.post('/api/stores/visit/:slug', authenticateUser, async (req, res) => {
  try {
    ...
    console.log('\n=== Store Visit Started ===');
    console.log('[Store Visit] Request details:', {
      slug,
      userId,
      headers: req.headers,
      method: req.method,
    ...
    console.log('\n[Store Visit] Initializing dataset...');
    const initializedStore = await initializeDataset(slug);
    
    if (!initializedStore) {
      console.error('[Store Visit] Failed to initialize dataset for slug:', slug);
      return res.status(404).json({ error: 'Store not found' });
    }

    console.log('[Store Visit] Store initialized:', {
      _id: initializedStore._id,
    ...
    const existingVisit = user.checkedInStore.find(s => s.storeId === initializedStore._id.toString());
    
    if (!existingVisit) {
      console.log('[Store Visit] Recording new visit');
      user.checkedInStore.push({
        storeId: initializedStore._id.toString(),
        checkedInAt: new Date(),
        title: initializedStore.title,
        slug: initializedStore.slug,
        // Add other relevant store data
      });
      await user.save();
      console.log('[Store Visit] Visit recorded for user:', userId);
    } else {
      console.log('[Store Visit] User already visited this store');
    }

    console.log('=== Store Visit Completed ===\n');
    res.json({ 
      success: true,
      store: {
        _id: initializedStore._id,
...
app.get('/api/stores/:slug', async (req, res) => {
  try {
    ...
    initializeDataset(slug).catch(error => {
...
initializeDataset(request.slug).catch(error => {
  console.error('Failed to start server:', error);
  process.exit(1);
});
