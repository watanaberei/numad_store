...
app.post('/api/stores/visit/:slug', authenticateUser, async (req, res) => {
  try {
    ...
    if (req.params.id) {
      ...
      stores = await Store.find({});
      console.log('[Store Visit] Found stores:', stores);
    }
    ...
    res.json(geojson);
    console.log('[Store Visit] GeoJSON:', geojson);
  } catch (error) {
    next(error);
  }
});
...
//     const storeData = new data.store();
//     // const contentfulStore = await storeData.store(slug);
//     console.log('[Store Visit] Contentful data:', storeData);

//     if (!contentfulStore) {
//       return res.status(404).json({ error: 'Store not found in Contentful' });
//     }

//     // 2. Get Yelp data if available
//     let yelpData = null;
//     if (contentfulStore.location) {
//       const yelpParams = {
...
//       isValidated: contentfulStore.isValidated || false,
//       storeAttributes: contentfulStore.storeAttributes || [],
//       hours: yelpData?.hours || contentfulStore.hours || [],
...
//         type: 'Store',
//         geolocation: {
//           lat: contentfulStore.location?.geolocation?.lat || yelpData?.coordinates?.latitude,
//           lon: contentfulStore.location?.geolocation?.lon || yelpData?.coordinates?.longitude
//         },
...
//         locatedIn: contentfulStore.location?.locatedIn
//       },
//       media: {
//         ...contentfulStore.media,
...
//     // Check if store already visited
...
const initializeServer = async () => {
  try {
    ...
    console.log('[Init] Store data:', storeData);
    
    // 3. Get test store data
    const storeSlug = storeData.slug;
    console.log('[Init] Store slug:', storeSlug);
    // Format store data
    const storeDoc = {
    ...
    try {
      ...
      console.log('[Init] Test store created/updated:', store._id);
    } catch (error) {
