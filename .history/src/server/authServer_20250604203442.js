...
// IMPROVED: Get user store data with better error handling
app.get('/api/user/store', authenticateToken, async (req, res) => {
  const userEmail = req.user.email;
  
  try {
    console.log(`[user-store] Fetching store data for user: ${userEmail}`);
    ...
    const recentCheckedInStores = user.checkedInStores ? user.checkedInStores.slice(-6).reverse() : [];
    const storeIds = [...new Set([currentStore, ...recentCheckedInStores.map(store => store.storeId)])].filter(Boolean);
    
    console.log('[user-store] Fetching stores:', storeIds);
    ...
    console.log(`[user-store] Returning data for ${storeData.length} stores`);
    ...
    console.error(`[user-store] Error fetching store data:`, error);
...
// IMPROVED: Export refresh token function for client use
...
httpServer.listen(authPort, () => {
  ...
  console.log(`  POST /verify-token - Verify token validity`);
  console.log(`  POST /refresh-token - Refresh access token`);
  console.log(`  GET /user/:username - Get user profile`);
  console.log(`  GET /profile - Get current user profile (authenticated)`);
  console.log(`  POST /settings - Update user settings (authenticated)`);
  console.log(`  GET /api/user - Get user data (authenticated)`);
  console.log(`  GET /api/user/store - Get user store data (authenticated)`);
  console.log(`  POST /api/user/store - Check in/out of stores (authenticated)`);
  console.log(`  POST /api/store/impression - Add store impression (authenticated)`);
});

///////////////////////// END SERVER INITIALIZATION /////////////////////////ching user profile:', error);
    res.status(500).json({ message: 'Server error' });
  }
});
