...
const initializeServer = async (slug) => {
  try {
    ...
    // console.log('[Init] Store details (storeData):', storeDetails);
    ...
    console.log('[Init] Store saved/updated in MongoDB:', store._id);
...
// Add current store endpoint
app.get('/api/stores/current', async (req, res) => {
  try {
    // Get the current store from query parameters
    const { slug } = req.query;
    console.log('[Current Store] Request query:', req.query);
    console.log('[Current Store] Slug from query:', slug);
    
    if (!slug) {
      console.log('[Current Store] No slug found in query parameters');
      return res.status(404).json({ error: 'No store slug found' });
    }
    ...
    if (!store) {
      console.log('[Current Store] Store not found for slug:', slug);
      return res.status(404).json({ error: 'Store not found' });
    }

    console.log('[Current Store] Found store:', {
      id: store._id,
      slug: store.slug,
      title: store.title
    });
    ...
    console.error('[Current Store] Error:', error);
...
app.get('/api/init-store/:slug', async (req, res) => {
  try {
    const { slug } = req.params;
    console.log('[init-store] Captured slug:', slug);
//     await initializeServer(slug);
//     res.json({ success: true });
//   } catch (error) {
...
// });
    ...
    if (!slug) {
      console.log('[init-store] No slug provided');
      return res.status(400).json({ error: 'No store slug provided' });
    }
    ...
    if (!storeDetails) {
      console.log('[init-store] Store not found for slug:', slug);
      return res.status(404).json({ error: 'Store not found' });
    }
    ...
    console.log('[init-store] Store initialized successfully:', {
      id: store._id,
      slug: store.slug,
      title: store.title
    });
...
const getStoreSlug = async () => {
  try {
      ...
      // const storeData = await makeServerRequest(`${API_URL}/api/stores/current`, 'GET');
      // console.log('[getStoreSlug] Got store data:', storeData.slug);
      // return storeData.slug;
    // Get the current store from the request URL
    const request = parseRequestUrl();
    console.log('[getStoreSlug] Parsed request URL:', request);
    
    if (!request.slug) {
      console.log('[getStoreSlug] No slug found in URL');
      return null;
    }

    return request.slug;
  } catch (error) {
    console.error('[getStoreSlug] Error getting store slug:', error);
    return null;
  }
};

// Update initServer function
const initServer = async () => {
  try {
    const slug = await getStoreSlug();
    // console.log('[initServer] Initializing with slug:', slug);
    // await initializeServer(slug);
    console.log('[initServer] Got slug:', slug);

    if (!slug) {
      console.log('[initServer] No slug available, waiting for client request');
      return;
    }
    // app.listen(port, () => {
    ...
    // Initialize store with the slug
    const initResponse = await fetch(`http://localhost:6000/api/init-store/${slug}`);
    if (!initResponse.ok) {
      throw new Error(`Failed to initialize store: ${initResponse.statusText}`);
    }

    const result = await initResponse.json();
    console.log('[initServer] Store initialized:', result);

  } catch (error) {
    console.error('[initServer] Error during initialization:', error);
    // Don't exit process, try with default store
    console.log('[initServer] Attempting initialization with default store...');
