...
app.post('/api/stores/visit/:slug', authenticateUser, async (req, res) => {
  try {
    ...
    // Use the same pattern as StoreScreen.js
...
const initializeServer = async () => {
  try {
    ...
    const testSlug = 'ca_orange-ca_los-angeles_long-beach_the-library';
    ...
    try {
      ...
      // Use the same pattern as StoreScreen.js
      const storeData = new data.StoreData();
      console.log('[Init] StoreData initialized');

      // Get store data using the same method as StoreScreen
      const freshStoreData = await storeData.store(testSlug);
      console.log('[Init] Got store data:', freshStoreData ? 'success' : 'not found');
      ...
      if (!store) {
        ...
        await store.save();
        console.log('[Init] Updated existing store:', store._id);
      }
...
app.get('/api/stores/:slug', async (req, res) => {
  try {
    ...
    if (!store) {
      ...
      const freshStoreData = await storeData.store(slug);
...
// Comment out the test insert for now since we have a working store
/*
// Update the test insert to check for existing store first
const testInsert = async () => {
  try {
    await connectDB(); 
    const existingStore = await Store.findOne({ slug: "test-store" });
    if (!existingStore) {
      ...
      console.log("Test store already exists, skipping insert");
    }
  } catch (error) {
