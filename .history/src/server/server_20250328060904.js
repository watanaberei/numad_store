...
import { makeServerRequest, parseRequestUrl, parseServerUrl } from './utils/utils.server.js';
...
// // Update the init-store endpoint
...
// Update getStoreSlug function
const getStoreSlug =  async (req, res)  => {
  try {
    // Get the current store from the request URL
    const parsedUrl = parseServerUrl(req.url);
    console.log('[getStoreSlug] Parsed URL:', parsedUrl);
...
const initServer = async (req) => {
  try {
    const slug = await getStoreSlug(req);
