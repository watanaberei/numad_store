...
import { UserModel, StoreModel, storeOperations, blogOperations, connectDB } from "./data/mongodb/mongodb.js";
...
///////////////////////// SYNC BLOGS AFTER AUTH /////////////////////////

// Add this function to sync user blogs after successful registration or login
async function syncUserBlogsAfterAuth(username) {
  try {
    console.log(`[AuthServer] Syncing blogs for new/returning user: ${username}`);
    ...
    if (userBlogs.length > 0) {
      console.log(`[AuthServer] Found ${userBlogs.length} blogs to sync for user ${username}`);
      
      // Get the user
      const user = await UserModel.findOne({ username });
      
      if (user) {
        // Check existing blogPosts
        const existingBlogIds = new Set(user.blogPosts.map(bp => bp.blogId?.toString()));
        let addedCount = 0;
        
        // Add any missing blogs
        ...
        if (addedCount > 0) {
          await user.save();
          console.log(`[AuthServer] Added ${addedCount} blogs to user ${username}'s blogPosts`);
        } else {
          console.log(`[AuthServer] User ${username} already has all their blogs synced`);
        }
      }
    } else {
      console.log(`[AuthServer] No blogs found for user ${username}`);
    }
  } catch (error) {
    console.error(`[AuthServer] Error syncing user blogs:`, error);
...
app.get('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    const user = await UserModel.findOne({ email: userEmail })
      ...
      .populate('blogPosts.blogId', 'title slug category snippet media status publishedAt'
      .maxTimeMS(5000);
      .lean();
    
    if (!user) {
      console.log(`[AuthServer] User not found in profile endpoint: ${userEmail}`);
    ...
    console.log(`[AuthServer] Complete profile found for: ${user.username}`);
     ...
     const stats = {
      ...
      totalImpressions: user.impressions?.length || 0,
      totalFollowers: user.followers?.length || 0,
      totalFollowing: user.following?.length || 0,
      totalBlogPosts: user.blogPosts?.filter(post => post.status === 'published').length || 0,
      totalBlogsCreated: user.blogsCreated?.filter(blog => blog.status === 'published').length || 0,
    ...
    const completeProfile = {
      ...
      blogPosts: user.blogsCreated || [],
      blogsCreated: user.blogsCreated || [],
      savedBlogs: user.blogsCreated || [],
      likedBlogs: user.blogsCreated || [],
      blogHistory: user.blogsCreated || [],
    ...
    // Sync any existing blogs for this user (in case they had blogs before registration)
    await syncUserBlogsAfterAuth(username);
    ...
    // Don't reference 'username' here as it may not be defined
