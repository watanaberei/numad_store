...
// Get store slug from StoreScreen request
try {
  const storeScreenRequest = await makeServerRequest('http://localhost:4000/api/store-screen');
  if (storeScreenRequest && storeScreenRequest.slug) {
    console.log('[Server] Retrieved slug from StoreScreen:', storeScreenRequest.slug);
    await initializeServer(storeScreenRequest.slug);
  } else {
    console.log('[Server] No slug found in StoreScreen request, using default initialization');
    await initializeServer(); 
  }
} catch (error) {
  console.error('[Server] Error getting slug from StoreScreen:', error);
  console.log('[Server] Using default initialization');
  await initializeServer();
}
