...
// import * as storeSchema from '../models/StoreModel.js';
...
export class StoreData {
  ...
  async getStoreBySlug(slug) {
    try {
      ...
      console.log(
        "[StoreData.getStoreBySlug] Valid stores:",
        validStores?.length
      );
      ...
      if (!store) {
        console.error(
          "[StoreData.getStoreBySlug] Store not found for slug:",
          slug
        );
        return null;
      }
      ...
      console.log(
        "[StoreData.getStoreBySlug] Combined store data:",
        combinedStore
      );
...
rating: yelp.businesses.rating || "0.00",
review_count: yelp.businesses.review_count || "0",
price: yelp.businesses.price || "$$",
costEstimate: "3-6",
storeType: contentful.category.genre || "Coffee Shop",
distance: "1.5mi",
city: yelp.businesses.location.city || "Cerritos",
state: yelp.businesses.location.state || "CA",
storeName: yelp.businesses.name || "Smoking Tiger Bread Factory",
distanceMiles: "1.1",
status: "Busy until 6pm",
galleryImages: [
  "https://s3-media0.fl.yelpcdn.com/bphoto/U-GfASZ4XZogkRBDW9-V8g/o.jpg",
  ...
  // Basic store information from Yelp
  storeName: yelp.businesses.name, // maps to yelp.name
  rating: yelp.businesses.rating,, // maps to yelp.rating
  review_count: yelp.businesses.review_count, // maps to yelp.review_count
  price: "", // maps to yelp.price (e.g. "$$")
  phone: "", // maps to yelp.display_phone
  
  // Location information
  address: "", // maps to yelp.location.address1
  city: "", // maps to yelp.location.city
  state: "", // maps to yelp.location.state
  zip_code: "", // maps to yelp.location.zip_code
  
  // Geolocation
  coordinates: {
    latitude: 0, // maps to yelp.coordinates.latitude
    longitude: 0 // maps to yelp.coordinates.longitude
  },

  // Distance (converted from meters to miles)
  distance: 0, // maps to yelp.distance
  distanceMiles: "0.0", // calculated from yelp.distance

  // Store type from categories
  categories: [], // maps to yelp.categories array
  storeType: "", // derived from first category.title

  // Status
  is_closed: false, // maps to yelp.is_closed
  status: {
    isOpen: false, // derived from yelp.hours[0].is_open_now
    currentDay: 0, // calculated
    currentTime: "", // calculated
    schedule: [] // transformed from yelp.hours[0].open
  },

  // Media
  photos: [], // maps to yelp.photos array
  image_url: "", // maps to yelp.image_url
  galleryImages: [], // combined from yelp.photos and image_url

  // Additional metadata
  url: "", // maps to yelp.url
  transactions: [], // maps to yelp.transactions
  costEstimate: "" // derived from yelp.price
};
