  ...
  export class StoreData {
    ...
    async getStoreBySlug(slug) {
      try {
        ...
        // Get store data
        const stores = await this.contentfulData.getStores();
        console.log("[StoreData.getStoreBySlug] All stores:", stores?.length);
        
        // Filter valid stores
        const validStores = stores.filter(store => store && store.slug);
        ...
        // Create store model instance
        const storeModel = StoreModel.fromData(store);
      
        const storeParams = {
          term: storeData.hero.storeName,
          location: storeData.hero.city + ', ' + storeData.hero.state,
        };
        console.log("searchRequest", storeParams);

        const Yelp = await Yelp.getStoreData(storeParams);
        console.log("Yelp", Yelp);


        
        // // Get Yelp data
        // const storeParams = storeModel.getYelpParams();
        // console.log("!!!!![StoreData.getStoreBySlug] storeParams:", storeParams);
        
        // const yelpData = await Yelp.getStoreData(storeParams);
        console.log("!!!!![StoreData.getStoreBySlug] Yelp data:", yelpData);
  
        // Combine store data with Yelp data
        const combinedData = {
          ...store,
          yelpData
        };
  
        console.log("[StoreData.getStoreBySlug] Combined store data:", combinedData);
        return combinedData;
...
//   }
  ...
  async getData() {
    let contentful = new ContentfulData();
// const socket = io('http://localhost:4000'); 
    ...
    const Yelp = await Yelp.getStoreData(searchRequest);
    console.log("Yelp", Yelp);
