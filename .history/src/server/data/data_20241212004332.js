...
import { title } from "../../client/components/components.js";
import YelpClient from "./yelp/yelpClient.js";
import { YelpApi } from "./yelp/yelpApi.js";
...
const YelpLocation = await YelpApi.transformBusinessData(businessId); 
console.log("Yelp data received:", YelpData);
console.log("Yelp location received:", YelpLocation);
// const YelpData = await YelpApi.getStoreData(businessId);
// //     console.log("Yelp data received:", YelpData);
// console.log("Loading data.js, YelpApi:", YelpClient);
...
export async function initializeData() {
  try {
    ...
    const businessLocation = "Cerritos, CA";

    console.log("Searching for business:", businessName, "in location:", businessLocation);


    // First search for the business
    const searchResults = await YelpApi.searchBusinesses(businessName, businessLocation);
    ...
    if (searchResults?.businesses?.[0]?.id) {
      ...
      const storeData = await YelpApi.getBusinessDetails(businessId);
      console.log("Store details:", storeData);

      if (!storeData?.hours?.[0]?.open) {
        console.warn("No hours data available in Yelp response");
        return null;
      }

      const hoursData = storeData.hours[0].open;
      const isCurrentlyOpen = storeData.hours[0].is_open_now;
      ...
      console.error("No business found for", businessName, businessLocation);
      return {
        storeData: store.item[0],
        searchResults: []
      };
...
//     // First search for the business
//     const searchResults = await YelpApi.searchBusinesses(searchParams);
//     console.log("Search results:", searchResults);
...
//     // const YelpData = await YelpApi.getStoreData('0fGRTbEhBNDUP7AfUFqvPQ'); // Using direct business ID
...
const test = async () => {
  const data = await YelpApi.getStoreData(
  ...
  item: [
    ...
    },
    {
      ...
      Location: {
        ...
        state: "CA",
        zip: "92369",
        attribute: location.locations[2].geotags
          ? location.locations[2].geotags
          : null
      },
      details: {
        rating: "3.33",
