...
export class StoreData {
  ...
  async getStoreBySlug(slug) {
    try {
      ...
      // Filter valid stores
      const validStores = storeData.filter(store => store && store.slug);
      debugLog("^03:getStoreBySlug", "Valid stores:", validStores.length);

      // Find matching store
      const store = validStores.find(s => s.slug === slug);
      debugLog("^04:getStoreBySlug", "Found matching store:", store);
      ...
      // 2. Get Yelp data
      const storeParams = {
        storeName: store.title?.trim(),
        storeAddress: store.address,
        storeCity: store.city || null,
        storeState: store.state || 'CA',
        storeLongitude: store.longitude || -117.924,
        storeLatitude: store.latitude || 33.8722
      };

      debugLog("^06:getStoreBySlug", "storeParams:", storeParams);
      
      const yelpData = await this.yelp.getStoreData(storeParams);
      debugLog("^08:getStoreBySlug", "Yelp data:", yelpData);

      // 3. Combine data
      const headerData = await this.buildHeaderData(store, yelpData);
      debugLog("^09:getStoreBySlug", "Created store object:", headerData);

      return {
        hero: Promise.resolve(headerData),
        overview: Promise.resolve(this.buildOverviewData(store, yelpData)),
        experience: Promise.resolve(this.buildExperienceData(store)),
        service: Promise.resolve(this.buildServiceData(store)),
        business: Promise.resolve(this.buildBusinessData(store, yelpData)),
        location: Promise.resolve(this.buildLocationData(store, yelpData))
      };
  ...
  async buildHeaderData(store, yelpData) {
    console.log("****hero[StoreData.getStoreBySlug]", "yelpData:", yelpData);
    
    // Ensure all data is properly resolved before returning
    return {
      header: {
        title: store.title,
  ...
  async getFoodData(getStoreBySlug) {
    try {
  ...
  async transformYelpHours(getStoreBySlug) {
    try {
      const yelpData = getStoreBySlug;
      console.log(
  ...
  async serviceCategoryData(data) {
    try {
  ...
  async overviewSummaryData(data) {
    try {
  ...
  async experience(yelpData) {
    try {
      const headerData = this.headerData(yelpData);
      console.log("*****headerData", headerData);
      const experience =
        {
          header: this.headerData?.experience ? this.headerData?.experience : null,
          
          area: this.areaData ? this.areaData : null,
  ...
  async location(yelpData) {
    try {
      const location =
        {
          header: this.headerData?.location ? this.headerData?.location : null,

          area: this.areaData ? this.areaData : null,
          attribute: this.location.locations ? this.location.locations : null,
      ...
      const service = [
        {
          ...
          // Ensure overviewSummaryData is provided or fallback to null
          attributes: this.attributesData ? this.attributesData : null,
          // Ensure textBlockData is provided or fallback to null
          text: this.textBlockData ? this.textBlockData : null,
  ...
  async store(slug) {
    try {
      ...
      const store = {
        ...
        service: this.service(yelpData),
        business: this.business(yelpData),
        location: this.location(yelpData),
        serviceCategoryData: this.serviceCategoryData(yelpData),
