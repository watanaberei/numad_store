...
export class StoreData {
  ...
  areaData(yelpData) {
    try {
      ...
      console.log("****areaData areaData:", areaData);
      return areaData;
    } catch (error) {
      console.error("getStoreBySlug", "Error:", error);
      return [];
  ...
  experience(yelpData) {
    try {
      ...
      // Get area data with proper structure for storeArea component
      const areaData = {
        item: [
          {
            area: "Outside",
            ...
            images: YelpData.photos?.map(photo => ({
              id: '',
              impressions: {
                users: '',
                likes: '',
      ...
      const experience = {
        header: {
          ...
          description: "Take a virtual tour of the store"
        },
        area: areaData,
        attribute: {
          bestfor: {
            title: "Best For",
            items: ["Working", "Meeting", "Studying"]
          },
          working: {
            title: "Working",
            items: ["WiFi", "Power Outlets", "Tables"]
          },
          environment: {
            title: "Environment",
            items: ["Quiet", "Spacious", "Clean"]
          },
          facility: {
            title: "Facility",
            items: ["Restroom", "Parking", "Accessible"]
          }
        },
        text: this.textBlockData ? this.textBlockData() : null,
...
// Add debug logging
address.city = function(address) {
  console.log("[DEBUG] Parsing city from address:", address);
  
  if (!address) {
    console.log("[DEBUG] Address is undefined");
    return null;
  }

  const matches = address.match(/([^,]+),\s*([A-Z]{2})/);
  console.log("[DEBUG] Address matches:", matches);

  if (matches && matches[1]) {
    const cityPart = matches[1].trim();
    console.log("[DEBUG] Extracted city:", cityPart);
    return cityPart;
  }

  console.log("[DEBUG] No city found in address");
  return null;
}
