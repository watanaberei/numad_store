// src/server/data/mongodb/mongodb.js (Updated to include BlogModel)
...
// Store operations for backwards compatibility
const storeOperations = {
  async syncStoreData(slug) {
    try {
      ...
      if (existingStore) {
        console.log(`[MongoDB] Store found: ${existingStore.title || slug}`);
      ...
      console.error(`[MongoDB] Error syncing store: ${error.message}`);
      return null;
    }
  },
  ...
  async getOrCreateStoreBySlug(slug) {
    try {
      ...
      console.error(`[MongoDB] Error getting/creating store: ${error.message}`);
      return null;
    }
  }
};
