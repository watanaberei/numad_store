...
const connectDB = async () => {
  try {
    ...
    const uri = process.env.MONGODB_URI || "mongodb+srv://user:sshkey@cluster0.bgd0ike.mongodb.net/test";
...
const storeOperations = {
  async findOrCreateStore(storeData) {
    try {
      let store = await StoreData.findOne({ 
        $or: [
  ...
  async updateStoreData(storeId, updates, source) {
    try {
      const store = await StoreData.findOne({ storeId });
      if (!store) {
  ...
  async syncStoreData(slug) {
    try {
      ...
      const existingStore = await StoreData.findOne({ slug });
      if (existingStore) {
      ...
      const savedStore = await StoreData.findOneAndUpdate(
        { slug: storeData.slug },
        storeDocument,
        { 
          upsert: true, 
  ...
  async getOrCreateStoreBySlug(slug) {
    try {
      ...
      let store = await StoreData.findOne({ slug });
...
export const storeData = {
  async upsertStore(storeData, source) {
    try {
      ...
      const store = await StoreData.findOneAndUpdate(
        { slug: storeData.slug },
        formattedData,
        { 
          upsert: true, 
