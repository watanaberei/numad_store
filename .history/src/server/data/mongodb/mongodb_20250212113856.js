...
const storeOperations = {
  ...
  async syncContentfulStore(contentfulData) {
    console.log("[MongoDB] Syncing Contentful store:", contentfulData.slug);
    try {
      const store = await Store.findOne({ slug: contentfulData.slug });
      ...
      if (store) {
        ...
        console.log("[MongoDB] Updated existing store:", store.slug);
      ...
      console.log("[MongoDB] Created new store:", newStore.slug);
      return await newStore.save();
    } catch (error) {
      console.error("[MongoDB] Error syncing Contentful store:", error);
      throw error;
    }
  },
  ...
  async syncYelpStore(storeId, yelpData) {
    console.log("[MongoDB] Syncing Yelp data for store:", storeId);
    try {
      const store = await Store.findOne({ storeId });
      if (!store) {
        console.error("[MongoDB] Store not found:", storeId);
        return null;
      }
      ...
      console.log("[MongoDB] Updated Yelp data for store:", store.slug);
      return await store.save();
    } catch (error) {
      console.error("[MongoDB] Error syncing Yelp store:", error);
      throw error;
    }
  },

  // Get store by slug with all data
  async getStoreBySlug(slug) {
    console.log("[MongoDB] Getting store by slug:", slug);
    try {
      const store = await Store.findOne({ slug })
        .populate('sections.service.history.userId')
        .populate('sections.summary.history.userId');
      
      if (!store) {
        console.log("[MongoDB] Store not found:", slug);
        return null;
      }

      console.log("[MongoDB] Found store:", store.slug);
      return store;
    } catch (error) {
      console.error("[MongoDB] Error getting store:", error);
      throw error;
    }
  }
};
...
export { User, Store, getDatabase, connectDB, storeOperations };