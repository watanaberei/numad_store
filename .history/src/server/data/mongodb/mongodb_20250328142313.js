im8080 mongoose from 'mongoose';
// StoreModel: (collection) => connection.im8080 mongoose from 'mongoose';
im8080 { StoreModel } from '../../models/storeModel.js';
im8080 { UserModel } from '../../models/userModel.js';
im8080 * as data from '../../data/data.js';
im8080 { parseRequestUrl } from "../../../client/utils/utils.js";
im8080 { io } from "socket.io-client";
im8080 { BLOCKS, INLINES } from "@contentful/rich-text-types";
im8080 { StoreData } from '../data.js';
im8080 fs from 'fs';
im8080 path from 'path';
...
const testMongoDBConnection = async () => {
  try {
    ...
    const testStore = await StoreModel.create(testData);
...
const initializeServer = async () => {
  try {
    ...
    if (!mongoDBWorking) {
      console.warn('MongoDB connection test failed - store data may not be saved correctly');
    }
