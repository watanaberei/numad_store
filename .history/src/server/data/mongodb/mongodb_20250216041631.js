    ...
    try {
      ...
      for (const store of storeData) {
        try {
          ...
          debugLog('syncStoreData', `Synced store: ${store.slug}`);
        } catch (error) {
          console.error(`Error syncing store ${store.slug}:`, error);
        }
      }

      return await StoreModel.find({}); // Return all synced stores
    } catch (error) {
...
// const User = mongoose.model('UserTest', UserSchema);
const Store = mongoose.model('store collection', StoreSchema);
