...
const testConnection = async () => {
  try {
    ...
    debugLog('testConnection', 'Store sync results:', syncResults);
  ...
  try {
    ...
    const results = await Promise.all(allStores.map(async (store) => {
      try {
        ...
        const result = await storeOperations.findOrCreateStore(storeDoc);
        debugLog('syncAllStores', `Synced store: ${result.slug}`);
        return result;
