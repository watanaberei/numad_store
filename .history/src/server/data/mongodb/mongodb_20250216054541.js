...
const storeOperations = {
  ...
  async updateStoreData(storeId, updates, source) {
    ...
    try {
      ...
      console.log('Got store data:', storeData?.length, 'stores');

      if (!storeData?.length) {
        throw new Error('No store data found');
      }

      // For each store in the data
      for (const store of storeData) {
        try {
          // Map all fields from the store object
          const storeDoc = {
            ...store,
            storeId: store.sys?.id || `store-${Date.now()}`,
            yelpData: store.yelpData || null
          };

          console.log('Saving store:', storeDoc.slug);

          // Use findOneAndUpdate to upsert
          const saved = await StoreModel.findOneAndUpdate(
            { slug: store.slug },
            storeDoc,
            { 
              upsert: true, 
          ...
          console.log('Saved store:', saved.slug);
        } catch (error) {
      ...
      console.error('syncStoreData error:', error);
      throw error;
    }
  },
  ...
  async getOrCreateStoreBySlug(slug) {
    try {
      debugLog('getOrCreateStoreBySlug', 'Checking store existence', { slug });
      
      // First check if store exists in MongoDB
      let store = await StoreModel.findOne({ slug });
      
      if (store) {
        debugLog('getOrCreateStoreBySlug', 'Found existing store', { id: store._id });
        return store;
      }

      // If store doesn't exist, get data from StoreData
      debugLog('getOrCreateStoreBySlug', 'Store not found, fetching from StoreData');
      const storeDataInstance = new StoreData();
      const storeData = await storeDataInstance.getStoreBySlug(slug);

      if (!storeData) {
        throw new Error('Store data not found in source');
      }

      // Format store data for MongoDB
      const storeDoc = {
        storeId: storeData.storeId || `store-${Date.now()}`,
        slug: storeData.slug,
        title: storeData.headline?.text || storeData.title,
        rating: storeData.yelpData?.rating,
        review_count: storeData.yelpData?.review_count,
        price: storeData.yelpData?.price,
        ...
        storeName: storeData.headline?.text,
        distanceMiles: storeData.distanceMiles,
        status: storeData.status,
        gallery: storeData.media?.gallery,
        ...
        metrics: {
          ...
          totalLikes: 0,
          totalDislikes: 0,
          averageRating: storeData.yelpData?.rating || 0
        }
      };

      // Create new store in MongoDB
      const newStore = new StoreModel(storeDoc);
      const savedStore = await newStore.save();
      
      debugLog('getOrCreateStoreBySlug', 'Created new store', { 
        id: savedStore._id,
      ...
      errorLog('getOrCreateStoreBySlug', error);
      throw error;
    }
  }
};
...
const syncAllStores = async () => {
  try {
    ...
    // Verify final count
    const finalCount = await StoreModel.countDocuments();
    console.log(`\n=== Sync Complete ===`);
...
const testStoreSync = async () => {
  try {
    ...
    // Get test store
    const storeData = new StoreData();
    const store = await storeData.getStoreBySlug('test-store-slug');
...
const testStoreStorage = async () => {
  try {
    ...
    const initialCount = await StoreModel.countDocuments();
    ...
    // Verify some data
    const sampleStore = await StoreModel.findOne({});
