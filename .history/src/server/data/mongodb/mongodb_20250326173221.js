...
const connectDB = async (retries = 5) => {
  try {
    ...
    const connection = await mongoose.connect("mongodb+srv://user:sshkey@cluster0.bgd0ike.mongodb.net/test", options);
    ...
    if (retries > 0) {
       console.log(`[MongoDB] Retrying connection... (${retries} attempts remaining)`);
...
const storeOperations = {
  async findOrCreateStore(storeData) {
    try {
      ...
      const slugFormats = [
        storeData.slug.toLowerCase().replace(/-/g, '_'),
        storeData.slug.toLowerCase().replace(/_/g, '-')
      ];
      ...
      const storeDoc = {
        ...
        slug: storeData.slug.toLowerCase().replace(/-/g, '_'), // Store with underscores
        sys: storeData.sys || {},
      ...
      return store;
       ...
       console.error('[MongoDB] Error in findOrCreateStore:', error.message);
      throw error;
    }
  },
  ...
  async syncStoreData(slug) {
    try {
      ...
      for (const slugFormat of slugFormats) {
        freshData = await storeData.store(slugFormat);
        if (freshData) break;
      }
