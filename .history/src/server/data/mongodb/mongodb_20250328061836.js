...
// storeSchema: (collection) => connection.import mongoose from 'mongoose';
...
const storeOperations = {
  async findOrCreateStore(storeData) {
    try {
      ...
      store = new storeSchema(storeData);
      await store.save();
      return store;
    } catch (error) {
  ...
  async updateStoreData(storeId, updates, source) {
    try {
      const store = await storeSchema.findOne({ storeId });
      if (!store) {
  ...
  async syncStoreData(slug) {
    try {
      ...
      const existingStore = await storeSchema.findOne({ slug });
      if (existingStore) {
      ...
      const savedStore = await storeSchema.findOneAndUpdate(
        { slug: storeData.slug },
        storeDocument,
        { 
          upsert: true, 
  ...
  async getOrCreateStoreBySlug(slug) {
    try {
      ...
      let store = await storeSchema.findOne({ slug });
