...
export class StoreData {
  ...
  async getStoreBySlug(slug) {
    try {
      ...
      return combinedStore, yelpStore;
    } catch (error) {
   ...
   heroData(getStoreBySlug) {
    const store = getStoreBySlug;
    console.log("?!?!heroDataheroData store:", store);
    const heroData = {
      // Fields used by storeDetail component
      rating: store?.rating || "0.00",
      review_count: store?.review_count || "0",
      price: store?.price || "$$",
      costEstimate: store?.price?.length?.toString() || "3-6",
      storeType: store?.categories?.[0]?.title || "Coffee Shop",
      ...
      storeName: store?.name || "Smoking Tiger Bread Factory",
      distanceMiles: store?.distance 
        ? (store.yelpData.distance / 1609.34).toFixed(1) 
        : "1.1",
      status: store?.hours?.[0]?.is_open_now ? "Open" : "Closed",
    ...
    console.log("?!?!heroData:", heroData.rating);
  }
  ...
  transformYelpHours(getStoreBySlug) {
    ...
    console.log("?????transformYelpHours!!!!![StoreData.getStoreBySlug] yelpData:", yelpData);
