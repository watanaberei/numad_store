...
export class StoreData {
  ...
  updateHeroData(yelpData) {
    try {
      console.log("[StoreData.updateHeroData] Updating hero data with:", yelpData);
      
      // Update heroData with Yelp information
      heroData.rating = yelpData.rating?.toString() || heroData.rating;
      heroData.review_count = yelpData.review_count?.toString() || heroData.review_count;
      heroData.price = yelpData.price || heroData.price;
      heroData.costEstimate = yelpData.price?.length?.toString() || heroData.costEstimate;
      heroData.storeType = yelpData.categories?.[0]?.title || heroData.storeType;
      heroData.distance = `${(yelpData.distance / 1609.34).toFixed(1)}mi` || heroData.distance;
      heroData.city = yelpData.location?.city || heroData.city;
      heroData.state = yelpData.location?.state || heroData.state;
      heroData.storeName = yelpData.name || heroData.storeName;
      heroData.status = yelpData.hours?.[0]?.is_open_now ? "Open" : "Closed";

      console.log("[StoreData.updateHeroData] Updated hero data:", heroData);
    } catch (error) {
      console.error("[StoreData.updateHeroData] Error updating hero data:", error);
    }
  }
}
...
const YelpData = StoreData.YelpData;
...
rating: YelpData.rating || "0.00",
review_count: YelpData.review_count || "0",
price: YelpData.price || "$$",
costEstimate: "3-6",
storeType: contentfulData.category.genre || "Coffee Shop",
distance: "1.5mi",
city: YelpData.location.city || "Cerritos",
state: YelpData.location.state || "CA",
storeName: YelpData.name || "Smoking Tiger Bread Factory",
distanceMiles: "1.1",
status: "Busy until 6pm",
galleryImages: [
  "https://s3-media0.fl.yelpcdn.com/bphoto/U-GfASZ4XZogkRBDW9-V8g/o.jpg",
