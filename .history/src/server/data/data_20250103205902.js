...
export class StoreData {
  ...
  async getStoreBySlug(slug) {
    try {
      ...
      console.log("Data.js Combined store data:", combinedStore);
...
export const data = await storeData.getData();
...
//     const validStores = storeData.filter((store) => data.slug);
//     console.log("data.js Valid stores:", validStores);
    
//     store = validStores.find((store) => data.slug === request.slug);
//     console.log("data.js slug", data.slug);
...
// const yelpData = await storeData.getData();
...
export async function initializeData() {
  try {
    ...
    const storeParams = {
      storeName: data.headline?.text,
      storeAddress: data.location?.address,
      storeCity: address.city(data.location?.address),
      storeState: address.state(data.location?.address)
    };
    console.log("!!!!![StoreData.getStoreBySlug] storeParams:", storeParams);
...
export const location = {
  locations:  [
    {
      city: 'Cerritos',
      // city: data?.item?.[0]?.location?.city,
      attrtags: [
        {
          title: 'Location',
          attributes: [
  ...
  [
    {
      ...
      location: {
        address: data?.location?.address1 || '12345 Main St.',
        city: data?.location?.city || 'Cerritos',
        area: 'Lincoln Station',
        state: data?.location?.state || 'CA',
        zip: data?.location?.zip_code || '90703',
        geolocation: {
        ...
        stats: {
          contributions: data?.review_count || 0,
