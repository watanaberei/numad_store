...
export class StoreData {
  ...
  async getStoreBySlug(slug) {
    try {
      ...
      globalThis.yelpStore = await this.yelp.getStoreData(storeParams);
      console.log('getStoreBySlug', 'Yelp data:', yelpStore);

      // 4. Combine the data and make store data globally available
      globalThis.combinedStore = {
        ...matchedStore,
        yelpData: yelpStore
          ? {
          ...yelpStore,
      ...
      store = combinedStore;
      console.log('getStoreBySlug', 'Setting global store data:', store);

      // Update heroData with the new store data
      this.heroData();

      console.log('getStoreBySlug', 'Combined store data:', combinedStore);
  ...
  sections(store) {
    this.object(store);
    const object = this.object(store);
    ...
    const sectionOverview = {
      header: object.objHeader.overview,
      summary: experienceScore,
      attributes: object.objAttributes,
      text: object.objTextBlock,
  ...
  // Export heroData with store-based values
    ...
    storeData(store) {
      const sectionHero = this.sections.sectionHero(store);
      return sectionHero;
    }
    

    // debugLog('sectionHero', 'Final heroData:', sectionHero);
  // }
...
contentful = {
  render: async () => {
    ...
    const storeData = this.getStoreBySlug.getData();
...
// export async function initializeData() {
  ...
  // 3. Get Yelp data using store details
  const yelpStore = getStoreData(storeParams);
  console.log('getStoreBySlug', 'Yelp data:', yelpStore);
   ...
   // 4. Combine the data and make store data globally available
    ...
    if (searchResults?.businesses?.[0]?.id) {
      ...
      const storeData = yelp.businessData(businessId);
      ...
      console.log('heroData', 'No store data available, using default values');
      // console('heroData', 'No store data available, using default values');
    }
...
heroData() {
  ...
  costEstimate= store?.yelpData?.price?.length?.toString() || "3-6",
  storeType= store?.yelpData?.categories?.[0]?.title || "Coffee Shop",
  distance= store?.yelpData?.distance ? `${(store.yelpData.distance / 1609.34).toFixed(1)}mi` : "1.5mi",
  city= store?.yelpData?.location?.city || "Cerritos",
  state= store?.yelpData?.location?.state || "CA",
  
  // Fields used by storeHeadline component
  storeName= store?.yelpData?.name || "Smoking Tiger Bread Factory",
  distanceMiles= store?.yelpData?.distance ? (store.yelpData.distance / 1609.34).toFixed(1) : "1.1",
  // status= store?.yelpData?.hours?.[0]?.is_open_now ? "Open" = "Closed",
...
store() {
  item: [
    {
      ...
      location: {
        address: yelpData?.location?.address1 || "12345 Main St.",
        city: yelpData?.location?.city || "Cerritos",
        area: "Lincoln Station",
        state: yelpData?.location?.state || "CA",
        zip: yelpData?.location?.zip_code || "90703",
        geolocation: {
  ...
  export const serviceCategoryData = StoreData.serviceCategoryData;
  export const footerData = StoreData.footerData;
  export const heroData = StoreData.heroData;
  export const headerData = StoreData.headerData;
  export const areaData = StoreData.areaData;
  export const attributesData = StoreData.attributesData;
  export const textBlockData = StoreData.textBlockData;
  export const overviewSummaryData = StoreData.overviewSummaryData;
  export const locationData = StoreData.locationData;
  export const businessData = StoreData.businessData;
  export const experienceData = StoreData.experienceData;
  export const serviceData = StoreData.serviceData;
  export const overviewData = StoreData.overviewData;
  export const mapRadiusData = StoreData.mapRadiusData;
