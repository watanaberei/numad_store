  ...
  heroData(storeData) {
    debugLog('heroData', 'Starting update with store:', storeData);
      ...
      const searchParams = {
        ...
        storeCity: address.city(storeData.location?.address),
        storeState: address.state(storeData.location?.address),
        storeLongitude: storeData.location?.geolocation?.lon,
        storeLatitude: storeData.location?.geolocation?.lat
      };

      // First search for the business
      const searchResults = await this.yelp.searchBusinesses(searchParams);
      ...
      if (searchResults?.businesses?.[0]?.id) {
        ...
        const storeDetails = yelp.businessData(businessId);
        debugLog('heroData', 'Store details:', storeData);
        yelpData = storeDetails.YelpData;
        if (storeDetails?.hours) {
          const hours = this.transformYelpHours(storeData);
