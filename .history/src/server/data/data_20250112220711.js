...
export class StoreData {
  ...
  // Helper function to transform Yelp hours data
  heroData(getStoreBySlug) {
    console.log("****heroData!!!!![StoreData.getStoreBySlug] yelpData:", getStoreBySlug);
    
    // Add defensive check for yelpData
    if (!getStoreBySlug || !getStoreBySlug.rating) {
      console.log("[(*****StoreData.heroData] No Yelp data available, using template");
      return {
        rating: this.store.item.rating || "3.33",
        review_count: this.store.item.reviewCount,
        price: this.store.item.price,
        costEstimate: this.store.item.costEstimate || "3-6",
        storeType: this.store.item.storeType || "Coffee Shop",
        storeName: this.store.item.storeName,
        status: this.store.item.status || "Closed",
        city: this.store.item.city,
        state: this.store.item.state,
        distance: this.store.item.distance || "1.5mi",
        distanceMiles: this.store.item.distanceMiles || "1.1",
        gallery: this.store.item.gallery
      };
    }
    ...
    const heroData = {
      rating: getStoreBySlug.rating,
      review_count: getStoreBySlug.review_count,
      price: getStoreBySlug.price,
      costEstimate: getCostEstimate(getStoreBySlug.price),
      storeType: getStoreBySlug.categories ? getStoreBySlug.categories.map(c => c.title) : ["Coffee Shop"],
      storeName: getStoreBySlug.name,
      status: getStoreBySlug.hours ? (getStoreBySlug.hours[0]?.is_open_now ? "Open" : "Closed") : "Closed",
      city: getStoreBySlug.location?.city,
      state: getStoreBySlug.location?.state,
      distance: getStoreBySlug.distance ? `${(getStoreBySlug.distance / 1609.34).toFixed(1)}mi` : "1.5mi",
      distanceMiles: getStoreBySlug.distance ? (getStoreBySlug.distance / 1609.34).toFixed(1) : "1.1",
      gallery: getStoreBySlug.photos
    };
