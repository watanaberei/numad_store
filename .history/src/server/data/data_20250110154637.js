...
export class StoreData {
  ...
  async getStoreBySlug(slug) {
    try {
      ...
      return combinedStore, storeParams, yelpData, contentfulData;
    } catch (error) {
  ...
  transformYelpHours() {
    const YelpData = storeData.yelpData.yelpData;
    console.log("!!!![StoreData.transformYelpHours] YelpData:", YelpData);
  ...
  heroData() {
    ...
    const store = StoreData.getStoreBySlug(slug);
    const heroData = {
  ...
  galleryImages: [
    ...
    "https://mo.tomasglobal.com/wp-content/uploads/2022/12/Smoking-Tiger-Cerritos-1.png"
      ...
      // Additional Yelp data fetching and processing
      ...
      const searchResults = this.yelp.searchBusinesses(storeData);
      debugLog('heroData', 'Search results:', searchResults);

      if (searchResults?.businesses?.[0]?.id) {
        // Then get business details using the ID
        const businessId = searchResults.businesses[0].id;
        const storeDetails = this.yelp.businessData(businessId);
        debugLog('heroData', 'Store details:', storeDetails);

        if (storeDetails?.hours) {
          const hours = this.transformYelpHours(storeDetails);
          debugLog('heroData', 'Transformed hours:', hours);
