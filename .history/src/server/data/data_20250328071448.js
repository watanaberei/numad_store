...
async getStoreBySlug(slug) {
  try {
    ...
    // 2. If no data returned, try to create a default store
    if (!storeData || storeData.length === 0) {
      console.warn(`[data.js] No store data available from DataPost`);
      return this.createDefaultStore(slug);
    }

    // 3. Filter for valid stores and find matching store
...
createDefaultStore(slug) {
  ...
  return {
    ...
    overview: [
      {
        header: "Overview",
        text: { title: "Summary", content: `Information about ${storeName} is currently being collected.` },
        footer: { contributionsCount: 0, modifiedDate: new Date().toLocaleDateString() }
      }
    ],
    experience: {
      header: "Experience",
