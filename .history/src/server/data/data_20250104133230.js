...
export const heroData = {
  ...
  rating: yelp.businesses?.[0]?.rating || "0.00",
  review_count: yelp.businesses?.[0]?.review_count || "0",
  price: yelp.businesses?.[0]?.price || "$$",
  costEstimate: yelp.businesses?.[0]?.price?.length || "3-6", // Derived from Yelp price level
  storeType: yelp.businesses?.[0]?.categories?.[0]?.title || "Coffee Shop",
  distance: `${(yelp.businesses?.[0]?.distance / 1609.34).toFixed(1)}mi` || "1.5mi", // Convert meters to miles
  city: yelp.businesses?.[0]?.location?.city || "Cerritos",
  state: yelp.businesses?.[0]?.location?.state || "CA",

  // Fields used by storeHeadline component
  storeName: yelp.businesses?.[0]?.name || "Smoking Tiger Bread Factory",
  distanceMiles: (yelp.businesses?.[0]?.distance / 1609.34).toFixed(1) || "1.1",
  status: yelp.businesses?.[0]?.hours?.[0]?.is_open_now ? "Open" : "Closed",

  // Fields used by heroGallery component
  galleryImages: [
    yelp.businesses?.[0]?.image_url,
    ...(yelp.businesses?.[0]?.photos || []),
    "https://mo.tomasglobal.com/wp-content/uploads/2022/12/Smoking-Tiger-Cerritos-1.png"
