...
export class StoreData {
  ...
  async getStoreBySlug(slug) {
    try {
      ...
      const yelpDataParams = {
        storeId: yelpData.storeId,
        storeAlias: yelpData.storeAlias,
        storeName: yelpData.storeName,
        storeCity: yelpData.storeCity,
        storeState: yelpData.storeState,
      }
      console.log("[StoreData.getStoreBySlug] Yelp data parameters:", yelpDataParams);
      
      const yelpFoodData = await this.yelp.getFoodData(yelpDataParams);
      const yelpFusionData = await this.yelp.getFusionData(yelpDataParams);
      const yelpSearchData = await this.yelp.getSearchData(yelpDataParams);
      const yelpPhoneData = await this.yelp.getPhoneSearchData(yelpDataParams);
      const yelpMatchData = await this.yelp.getBusinessMatchData(yelpDataParams);
      const yelpDetailsData = await this.yelp.getBusinessDetailsData(yelpDataParams);
      const yelpDeliveryData = await this.yelp.getFoodDeliverySearchData(yelpDataParams);
      const yelpServiceData = await this.yelp.getServiceOfferingsData(yelpDataParams);
      const yelpInsightData = await this.yelp.getBusinessInsightsData(yelpDataParams);
...
const neighborhoodAttributes =
  ...
  async store(slug) {
    try {
      ...
      console.log("^05:[StoreData.getStoreBySlug] matchedStore:", matchedStore);
      const storeParams = {
        storeName: matchedStore.headline?.text,
        storeAddress: matchedStore.location?.address,
        storeCity: address.city(matchedStore.location?.address),
