...
export class StoreData {
  ...
  async getStoreBySlug(slug) {
    try {
      ...
      if (!matchedStore) {
        console.error("!03:getStoreBySlug", "Store not found for slug:", slug);
        return null;
      }
      // const yelpData = await this.yelp.getStoreData({
      ...
      // 4. Build the store sections
      const store = {
        hero: {
          rating: yelpData.rating,
          review_count: yelpData.review_count,
          price: yelpData.price,
          costEstimate: yelpData.costEstimate || "$9-12",
          ...
          status: "Open", // You might want to get this from the API
          gallery: yelpData.photos || []
        },
        overview: [{
          header: matchedStore.overview?.header || null,
          summary: {
            card: matchedStore.overview?.summary?.card || null,
            text: matchedStore.overview?.summary?.text || null
          },
          footer: {
            contributionsCount: matchedStore.overview?.footer?.contributionsCount || 0,
            modifiedDate: matchedStore.overview?.footer?.modifiedDate || 'N/A',
            modifiedTime: matchedStore.overview?.footer?.modifiedTime || 0,
            commentsCount: matchedStore.overview?.footer?.commentsCount || 0,
            reviewsCount: yelpData.review_count || 0,
            likesCount: matchedStore.overview?.footer?.likesCount || 0,
            dislikesCount: matchedStore.overview?.footer?.dislikesCount || 0
          }
        }],
        experience: {
          header: matchedStore.experience?.header || 'Experience',
          area: matchedStore.experience?.area || [],
          attribute: matchedStore.experience?.attribute || {},
          text: matchedStore.experience?.text || {},
          footer: matchedStore.experience?.footer || {}
        }
      };
  ...
  location(yelpData) {
    try {
      ...
      const storeLocation = {
        ...
        city: address.city(YelpData.location?.address),
        state: address.state(YelpData.location?.address),
        storeFullAddress: address + ', ' + city + ', ' + state
      };
