...
export async function test() {
  try {
    console.log("[data.js] Starting test function");
    const businessId = '0fGRTbEhBNDUP7AfUFqvPQ';
    const storeData = await yelp.getStoreData(businessId);
    console.log("[data.js] Store data received:", storeData);
    return storeData;
  } catch (error) {
    console.error("[data.js] Test function error:", error);
    throw error;
  }
}
...
export async function initializeData() {
  try {
    ...
    if (yelpData?.hours) {
      const hoursData = yelp.businessData.transformedData.hours(yelpData);
  ...
  [
    {
      ...
      location: {
        address: yelpApi?.location?.address1 || '12345 Main St.',
        city: yelpApi?.location?.city || 'Cerritos',
        area: 'Lincoln Station',
        state: yelpApi?.location?.state || 'CA',
        zip: yelpApi?.location?.zip_code || '90703',
        geolocation: {
          lat: yelpApi?.coordinates?.latitude,
          lon: yelpApi?.coordinates?.longitude
        },
        attribute: {
          city: yelpApi?.location?.city || 'Cerritos',
          attrtags: [
        ...
        stats: {
          contributions: yelpApi?.review_count || 0,
          reviews: yelpApi?.review_count || 0,
          comments: 0,
          likes: 0,
          dislikes: 0
        },
