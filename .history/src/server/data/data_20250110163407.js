...
export class StoreData {
  ...
  heroData(getStoreBySlug) {
    ...
    const price = YelpData.price;
    const costEstimate = getCostEstimate.render(price);
    ...
    return {
      ...
      price: price || "NA",
      costEstimate: store?.price?.length?.toString() || "NA",
      storeType: store?.categories?.[0]?.title || "NA",
      distance: store?.distance
        ? `${(store.yelpData.distance / 1609.34).toFixed(1)}mi`
        : "NA",
      city: store?.location?.city || "NA",
      state: store?.location?.state || "NA",

      // Fields used by storeHeadline component
      storeName: store?.name || "NA",
