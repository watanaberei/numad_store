...
if (typeof window === 'undefined') {
  ...
  console.error('dotenv is not supported in the browser environment');
}
...
class YelpClient {
  ...
  async fetchYelpAPI(endpoint, params = {}) {
    ...
    const url = `${process.env.YELP_BASE_URL}${endpoint}${queryString ? '?' + queryString : ''}`;
  ...
  async setCache(key, value, expiresIn = 3600000) { // Default 1 hour
    try {
      const item = {
        value,
  ...
  async getBusinessDetails(businessId) {
    ...
    try {
      // Try cache first
      const cached = await this.render(cacheKey);
      if (cached) {
...
//   async fetchYelpAPI(endpoint, params = {}) {
//     const queryString = new URLSearchParams(params).toString();
//     const url = `${getConfig.YELP_BASE_URL}${endpoint}${queryString ? '?' + queryString : ''}`;
...
//       const response = await fetch(url, {
//         headers: {
//           'Authorization': `Bearer ${getConfig.YELP_API_KEY}`,
//           'Accept': 'application/json',
//           'Content-Type': 'application/json'
//         }
//       });
