...
class YelpClient {
  ...
  async searchBusinesses(params = {}) {
    ...
    try {
      ...
      const data = await fetchYelpAPI("/businesses/search", params);
      await setCache(cacheKey, data, 1800000); // 30 minutes cache for search
      return data;
    } catch (error) {
  ...
  async getBusinessReviews(businessId) {
    ...
    try {
      ...
      const data = await fetchYelpAPI(`/businesses/${businessId}/reviews`);
      await setCache(cacheKey, data, 3600000); // 1 hour cache for reviews
      return data;
    } catch (error) {
