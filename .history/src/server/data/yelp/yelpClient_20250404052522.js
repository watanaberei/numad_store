...
class YelpClient {
  ...
  async getBusinessFood(storeId) {
    const cacheKey = `yelp:business:${storeId}`;

    try {
      // Try cache first
      const cached = await this.getCached(cacheKey);
      if (cached) {
        console.log("Cache hit for business", storeId);
        return cached;
      }

      // Fetch from API
      const data = await this.fetchYelpAPI(`/businesses/${storeId}/insights/food_and_drinks`);
      console.log("YelpClient.getBusinessFood", data);
      await this.setCache(cacheKey, data);
      return data;
    } catch (error) {
      console.log("Error fetching business details", error);
