...
export class Yelp {
  ...
  //     console.log("[Yelp.getStoreData] Business ID:", businessId);
  //     const businessData = await yelpApi.getBusinessDetails(businessId);
  //     const foodData = await yelpApi.getBusinessFood(businessId);
  //     console.log("[Yelp.getBusinessFood] Food data:", foodData);
  //     // console.log("[Yelp.getStoreData] Business details:", businessData);
  ...
  static async getStoreData(store) {
    try {
      ...
      if (store.yelpId) {
        console.log("[Yelp.getStoreData] Using existing Yelp ID:", store.yelpId);
        businessId = store.yelpId;
        businessData = await yelpApi.getBusinessDetails(businessId);
      } else {
        // Otherwise, search for the business
        const storeName = store.storeName;
        const storeCity = store.storeCity;
        ...
        // Get detailed business data using ID from search results
        businessId = yelpData.businesses[0].id;
        console.log("[Yelp.getStoreData] Business ID:", businessId);
        businessData = await yelpApi.getBusinessDetails(businessId);
      }
      ...
      // Return only the business data - food data will be fetched separately
      return businessData;
  ...
  //   try {
  //     console.log("[Yelp.getFoodData] Starting with store:", store);
      
  //     // const storeId = store.getBusinessDetails(businessId);

  //     // // If we have a Yelp ID, use it directly
  //     // if (storeParams.yelpId) {
  //     //   console.log("[Yelp.getFoodData] Using existing Yelp ID:", storeParams.yelpId);
  //     //   const foodData = await yelpApi.getBusinessFood(storeParams.yelpId);
  //     //   // console.log("[Yelp.getStoreData] Business details:", businessData);
  //     //   return foodData;
  //     // }
  ...
  //     //   storeAlias: storeParams.storeAlias,
  ...
  //     // const yelpFoodData = await yelpApi.getBusinessFood(storeId);

  //     const yelpFoodData = await yelpApi.getBusinessFood(store.storeId);

  //     // const foodData = await yelpApi.getFoodData(searchParams.storeId);
  //     console.log("[Yelp.getFoodData] Food data received:", yelpFoodData);
  //     return yelpFoodData;
  ...
  //     // If we have a Yelp ID, use it directly
  //     if (store.yelpId) {
  //       console.log("[Yelp.getFoodData] Using existing Yelp ID:", store.yelpId);
  //       const foodData = await yelpApi.getBusinessFood(store.yelpId);
  //       return foodData;
  //     }
      
  //     // If we have a store with yelpData containing an id, use that
  //     if (store.yelpData && store.yelpData.id) {
  //       console.log("[Yelp.getFoodData] Using ID from yelpData:", store.yelpData.id);
  //       const foodData = await yelpApi.getBusinessFood(store.yelpData.id);
  //       return foodData;
  //     }
  
  //     // If we just have a store ID, use that
  //     if (store.id) {
  //       console.log("[Yelp.getFoodData] Using store ID:", store.id);
  //       const foodData = await yelpApi.getBusinessFood(store.id);
  //       return foodData;
  //     }
  
  //     // Otherwise, search for the business first to get an ID
  //     const storeName = store.storeName;
  //     const storeCity = store.storeCity;
  //     const storeState = store.storeState;
  //     const storeLocation = storeCity + ', ' + storeState;
  ...
  //     if (!yelpData?.businesses?.[0]?.id) {
  //       throw new Error('No matching business found for food data');
  //     }
  ...
  static async getFoodData(store) {
    try {
      ...
      // Check if we already have food data (possibly from a previous fetch)
      if (store.yelpFoodData) {
        console.log("[Yelp.getFoodData] Using existing food data");
        return store.yelpFoodData;
      }
      ...
      if (store.yelpId) {
        console.log("[Yelp.getFoodData] Using existing Yelp ID:", store.yelpId);
        businessId = store.yelpId;
      } else if (store.yelpData && store.yelpData.id) {
        console.log("[Yelp.getFoodData] Using ID from yelpData:", store.yelpData.id);
        businessId = store.yelpData.id;
      } else if (store.id) {
        console.log("[Yelp.getFoodData] Using store ID:", store.id);
        businessId = store.id;
      } else {
        // If we don't have an ID, return mock data to avoid rate limiting issues
        console.log("[Yelp.getFoodData] No ID available, returning mock data");
      ...
      try {
        ...
        // If we got a rate limit error or empty data, return mock data
        if (foodData.error || foodData.limited || !foodData.food_and_drinks) {
          console.log("[Yelp.getFoodData] Returning mock data due to API limitations");
          return this.getMockFoodData(businessId);
        }
  ...
  // Add a helper method to provide mock food data when rate limited
  static getMockFoodData(businessId) {
    return {
      mock: true,
