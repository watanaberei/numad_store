...
export class Yelp {
  static async getStoreData(store) {
    try {
      ...
      const businessData = await yelpApi.getBusinessDetails(storeId);
      const foodData = await yelpApi.getBusinessFood(storeId);
    ...
    try {
      console.log("[Yelp.getFoodData] Starting with params:", storeParams);
            ...
            if (storeParams.storeId) {
              console.log("[Yelp.getFoodData] Using existing Yelp ID:", storeParams.storeId);
              const foodData = await yelpApi.getBusinessFood(storeParams.storeId);
              // console.log("[Yelp.getStoreData] Business details:", businessData);
              return foodData;
            }
      ...
      const yelpFoodData = await yelpApi.getBusinessFood(storeParams.storeId);
  ...
  static async getSearchData(storeParams) {
    try {
      ...
      // If we have a Yelp ID, use it directly
      if (storeParams.storeId) {
        console.log("[Yelp.getStoreData] Using existing Yelp ID:", storeParams.storeId);
        const businessData = await yelpApi.getBusinessDetails(storeParams.storeId);
        // console.log("[Yelp.getStoreData] Business details:", businessData);
        return businessData;
      }
