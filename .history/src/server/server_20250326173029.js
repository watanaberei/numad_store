...
const initializeServer = async (slug) => {
  try {
    ...
    if (!storeDetails) {
      ...
      const hyphenatedSlug = slug.replace(/_/g, '-').toLowerCase();
      console.log('[Init] Trying hyphenated slug:', hyphenatedSlug);
      const storeDetailsAlt = await storeData.store(hyphenatedSlug);
      
      if (!storeDetailsAlt) {
        throw new Error('Failed to fetch initial store data');
      }
      
      console.log('[Init] Found store with hyphenated slug');
      storeDetails = storeDetailsAlt;
    }
...
const getStoreSlug = async () => {
  try {
    ...
    return storeData?.slug || DEFAULT_STORE_SLUG;
  } catch (error) {
    console.error('[Server] Error getting store slug:', error.message);
    // Always return the default slug if request fails
  ...
  try {
    ...
    if (!app.listening) {
      app.listen(port, () => {
        ...
        console.log(`[Server] Store initialized with slug: ${slug}`);
      });
    }
    ...
    await initializeServer(DEFAULT_STORE_SLUG);
  }
};

// Update the current store endpoint
app.get('/api/stores/current', async (req, res) => {
  try {
    ...
    console.error('[API] Error getting current store:', error.message);
    res.status(500).json({ error: error.message });
  }
});
...
app.get('/api/stores/:slug', async (req, res) => {
  try {
    ...
    console.log('[API] Fetching store:', slug);

    // Try both formats of the slug
    const slugFormats = [
      slug.toLowerCase().replace(/-/g, '_'), // with underscores
      slug.toLowerCase().replace(/_/g, '-')  // with hyphens
    ];

    // Try to find store in MongoDB
    let store = null;
    for (const slugFormat of slugFormats) {
      store = await Store.findOne({ slug: slugFormat });
      if (store) break;
    }
    
    if (!store) {
      // If not found, try to get from data.js
      const storeData = new data.StoreData();
      let storeDetails = null;
      
      for (const slugFormat of slugFormats) {
        storeDetails = await storeData.store(slugFormat);
        if (storeDetails) break;
      }
      ...
      // Save to MongoDB
      store = await Store.create(storeDetails);
      console.log('[API] Created new store:', store.slug);
    }
    
    res.json(store);
  } catch (error) {
    console.error('[API] Error getting store:', error.message);
    res.status(500).json({ error: error.message });
  }
});
