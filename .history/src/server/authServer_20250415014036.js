...
// Store check-in endpoint
// app.post('/api/store/visits', authenticateToken, async (req, res) => {
...
//       if (store) {
//         // Update the store's visits count if interactions field exists
//         if (store.interactions) {
//           store.interactions.visitss = (store.interactions.visitss || 0) + 1;
//           await store.save();
//         }
//       }
...
app.post('/api/store/visited', authenticateToken, async (req, res) => {
  const { storeId, action } = req.body;
  const userEmail = req.user.email;
  ...
  try {
    ...
    if (action === 'visit') {
      ...
      // Update the store's visit count
      if (store.interactions) {
        store.interactions.visits = (store.interactions.visits || 0) + 1;
      }
      ...
      console.log(`[authServer] User ${userEmail} visited store ${storeId}`);
      return res.status(200).json({ 
        success: true, 
        message: 'Checked in successfully',
        storeId: storeId
...
app.get('/api/store/visited/status', authenticateToken, async (req, res) => {
  try {
    const user = await UserModel.findOne({ email: req.user.email });
    if (!user) {
      return res.status(404).json({ success: false, message: 'User not found' });
