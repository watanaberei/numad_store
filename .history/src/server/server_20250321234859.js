...
const initializeDataset = async (slug) => {
  try {
    console.log('[Dataset] Initializing dataset for slug:', slug);
    
    // 1. Get store data
    const storeData = new data.StoreData();
    const storeDetails = await storeData.store(slug);
    console.log('[Dataset] Store details fetched:', storeDetails?.title);
    ...
    console.log('[Dataset] Store saved/updated:', {
      _id: store._id,
      slug: store.slug,
      title: store.title
    });
...
// Update the store endpoints to use initializeDataset
app.get('/api/stores/:slug', async (req, res) => {
  try {
    ...
    // If not found or stale, initialize dataset
    if (!store || Date.now() - store.updatedAt > 24 * 60 * 60 * 1000) {
