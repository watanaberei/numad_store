...
function authenticateUser (req, res, next) {
 ...
 app.post('/api/blog', authenticateUser, async (req, res) => {
  try {
    ...
    const blogData = {
      slugBlog: slugBlog, // Use slugBlog to match your model
      title,
      ...
      author: {
        slugAuthor: req.user.slugAuthor,
...
app.post('/api/blog/:slug', authenticateUser, async (req, res) => {
  try {
    const { slugBlog } = req.params;
    console.log('[Blog API] Updating blog:', slugBlog);
    
    const blog = await BlogModel.findOne({ slugBlog: slugBlog }); // Use slugBlog
...
// Fix the GET blog route to use slugBlog
app.get('/api/blog/:slug', async (req, res) => {
  try {
    const blog = await BlogModel.findOne({ slugBlog: req.params.slugBlog }); // Use slugBlog
...
// Fix the GeoJSON endpoint to use slugBlog
  ...
  try {
    ...
    const geojsonBlogs = {
      ...
      features: blogs.map(blog => ({
        ...
        properties: {
          variant: 'blogs',
          slugBlog: blog.slugBlog, // Use slugBlog
          title: blog.title,
