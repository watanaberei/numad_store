...
UserSchema.methods.getStats = function() {
  return {
    places: (this.visitHistory?.length || 0) + (this.savedStores?.length || 0),
    contributions: (this.impressionsLiked?.length || 0) + (this.impressionsDisliked?.length || 0),
    followers: this.followers?.length || 0,
    following: this.following?.length || 0,
    reviews: this.impressionsLiked?.length || 0,
    ///////////////////////// START BLOG STATS /////////////////////////
    blogPosts: this.blogPosts ? this.blogPosts.filter(p => p.status === 'published').length : 0,
    totalBlogs: this.blogsCreated?.length || 0,
    ///////////////////////// END BLOG STATS /////////////////////////
  };
};
...
UserSchema.methods.getProfileStats = function() {
  ...
  const recentCheckIns = this.checkedInStores?.filter(c => 
    new Date(c.checkedInAt) > thirtyDaysAgo
  ).length || 0;
  
  const recentReviews = this.impressions?.filter(i => 
    new Date(i.createdAt) > thirtyDaysAgo
  ).length || 0;
  
  ///////////////////////// START RECENT BLOG POSTS /////////////////////////
  const recentBlogPosts = this.blogPosts?.filter(p => 
    p.status === 'published' && new Date(p.publishedAt) > thirtyDaysAgo
  ).length || 0;
  ///////////////////////// END RECENT BLOG POSTS /////////////////////////
  
  return {
    totalPlaces: this.visitHistory?.length || 0,
    savedPlaces: this.savedStores?.length || 0,
    totalReviews: this.impressions?.length || 0,
    totalFollowers: this.followers?.length || 0,
    totalFollowing: this.following?.length || 0,
    ...
    ///////////////////////// START BLOG COUNTS /////////////////////////
    blogCounts: {
      published: this.blogPosts?.filter(p => p.status === 'published').length || 0,
      draft: this.blogPosts?.filter(p => p.status === 'draft').length || 0,
      archived: this.blogPosts?.filter(p => p.status === 'archived').length || 0,
      total: this.blogPosts?.length || 0
    }
    ///////////////////////// END BLOG COUNTS /////////////////////////
  };
};
...
// FIXED: Cache implementation for frequently accessed users
const userCache = new Map();
const USER_CACHE_TTL = 60000; // 1 minute

// FIXED: Post-find middleware to implement caching
UserSchema.post('findOne', function(doc) {
  if (doc && doc.username) {
    console.log(`[UserModel] Caching user: ${doc.username}`);
    userCache.set(doc.username, {
      data: doc,
      timestamp: Date.now()
    });
  }
...
// FIXED: Clean up old cache entries periodically
setInterval(() => {
  const now = Date.now();
  ...
  if (entriesToDelete.length > 0) {
    console.log(`[UserModel] Cache cleanup: Removed ${entriesToDelete.length} entries. Current cache size: ${userCache.size}`);
  }
...
console.log('[UserModel] User model loaded successfully with blog posts integration');
