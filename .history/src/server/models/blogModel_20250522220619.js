...
BlogSchema.pre("save", function (next) {
  console.log(`[MongoDB] Saving blog: ${this.BlogSlug}`);
  this.updatedAt = new Date();
  
  // Auto-generate BlogSlug if not provided
  if (!this.BlogSlug && this.title) {
    this.BlogSlug = this.title
      .toLowerCase()
      .replace(/[^\w\s]/gi, '')
      .replace(/\s+/g, '-')
      .substring(0, 50);
...
BlogSchema.post("save", function (doc) {
  console.log(`[MongoDB] Blog saved: ${doc.BlogSlug} (ID: ${doc._id})`);
});

// Post-update middleware
BlogSchema.post("findOneAndUpdate", function (doc) {
  if (doc) {
    console.log(`[MongoDB] Blog updated: ${doc.BlogSlug} (ID: ${doc._id})`);
  }
});
...
BlogSchema.statics.findByBlogSlug = function(BlogSlug) {
  return this.findOne({ BlogSlug })
    .populate('createdBy', 'email firstName lastName')
...
BlogSchema.statics.getRelated = function(blogId, category, limit = 3) {
  ...
  .sort({ publishedAt: -1 })
  .limit(limit)
  .select('title BlogSlug media.thumbnail category publishedAt')
  .lean();
};
