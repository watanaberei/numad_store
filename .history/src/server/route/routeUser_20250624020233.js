...
routeUser.get('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`[UserAPI] Fetching complete profile for: ${userEmail}`);
    
    const user = await UserModel.findOne({ email: userEmail })
      .select('-password -twoFactorSecret -apiKeys')
      .maxTimeMS(10000);
      
    if (!user) {
      console.log('[UserAPI] User not found in profile endpoint');
    ...
    const completeProfile = {
        ...
        storesCreated: user.storesCreated || [],
...
routeUser.put('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`[UserAPI] Updating profile for: ${userEmail}`);
    ...
    console.log(`[UserAPI] Profile updated for: ${user.username}`);
