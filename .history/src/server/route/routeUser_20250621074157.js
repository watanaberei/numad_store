...
routeUser.get('/@:username/activity', async (req, res) => {
  try {
    ...
    const recentActivity = user.getRecentActivity(parseInt(limit));
...
// Legacy endpoint
routeUser.get('/user-profile', authenticateToken, async (req, res) => {
...
// Legacy endpoint
routeUser.post('/user-profile', authenticateToken, async (req, res) => {
  // Redirect to new endpoint
  console.log('[UserAPI] Legacy endpoint accessed, redirecting to /profile');
  return req.app.handle(req, res);
});
