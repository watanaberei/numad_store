...
routeUser.get('/user/:username', async (req, res) => {
  try {
    ...
    console.log(`/user/:username [UserAPI] Fetching public profile for username: ${user.username}`);
    ...
    .maxTimeMS(10000);
    console.log(`/user/:username [UserAPI] User found: ${user.username}`);
    
    if (!user) {
      console.log(`/user/:username [UserAPI] User not found: ${user.username}`);
      return res.status(404).json({ message: 'User not found' });
    }
    
    console.log(`/user/:username [UserAPI] Public profile found for: ${user.username}`);
...
routeUser.post('/user/:username/follow', authenticateToken, async (req, res) => {
  try {
    ...
    const [currentUser, targetUser] = await Promise.all([
      ...
      UserModel.findOne({ username: username.toLowerCase(), isActive: true })
    ]);
