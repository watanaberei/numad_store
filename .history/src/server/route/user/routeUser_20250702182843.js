...
const authenticateToken = async (req, res, next) => {
  ...
  try {
    ...
    req.user = decoded;
    console.log('[UserAPI] Token verified for user:', decoded.email);
    next();
  } catch (error) {
    console.error('[UserAPI] Token verification failed:', error.message);
...
routeUser.get('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    const completeProfile = {
      ...
      receivedEndorsements: user.receivedEndorsements,
    ...
    console.error('[UserAPI] Error fetching complete profile:', error);
...
routeUser.get('/@:username', async (req, res) => {
  try {
    ...
    console.log(`[UserAPI] Fetching public profile for: ${username}`);
...
//       const user = await UserModel.findOne({ email: userEmail })
//         .select('email username checkedInStore checkedInStores visitHistory savedStores profilePrivacy blogPosts blogsCreated savedBlogs likedBlogs')
//         .maxTimeMS(5000);
...
routeUser.get('/user/blogs', authenticateToken, async (req, res) => {
  try {
    ...
    const user = await UserModel.findOne({ email: userEmail })
      .select('username blogPosts blogsCreated savedBlogs likedBlogs blogHistory')
      .populate({
    ...
    const authoredBlogs = await UserModel.find({
...
routeUser.get('/user/:username/blogs', async (req, res) => {
  try {
    ...
    console.log('[UserAPI] Getting blog data for:', username, 'tab:', tab);
...
routeUser.post('/user/:username/follow', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('[UserAPI] Follow/unfollow request:', { from: userEmail, to: targetUsername, follow });
    
    // Find both users
    const [currentUser, targetUser] = await Promise.all([
      UserModel.findOne({ email: userEmail }),
      UserModel.findOne({ username: targetUsername.toLowerCase() })
    ]);
    ...
    const followingIndex = currentUser.following.findIndex(f => 
      f.userId.equals(targetUser._id)
    );
    const followerIndex = targetUser.followers.findIndex(f => 
    ...
    if (follow) {
      ...
      console.log('[UserAPI] User followed successfully');
      res.json({
        success: true,
      ...
      console.log('[UserAPI] User unfollowed successfully');
      res.json({
        success: true,
