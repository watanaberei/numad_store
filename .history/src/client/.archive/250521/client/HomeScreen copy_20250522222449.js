...
after_render: async () => {
  ...
  // Load both store and blog data
  const { features: storeFeatures } = await geojsonStore();
  const blogFeatures = await loadBlogFeatures();
...
async function loadBlogFeatures() {
  try {
    ...
    if (response.ok) {
      ...
      console.error('Failed to load blog features');
      return [];
    }
  } catch (error) {
    console.error('Error loading blog features:', error);
    return [];
  }
}
...
function renderFeatures(features, map) {
  ...
  features.forEach((feature) => {
    ...
    let element;
    const containerName = `post${feature.properties.variant.charAt(0).toUpperCase() + feature.properties.variant.slice(1)}`;
    ...
    switch (feature.properties.variant) {
      case "stores":
        element = createGeojsonStoreListing(feature, map, getUserCoordinates());
        break;
      case "reviews":
        element = createGeojsonReviewListing(feature, map, getUserCoordinates());
        break;
      case "articles":
        element = createGeojsonArticleListing(feature, map, getUserCoordinates());
        break;
      case "blogs":
        element = createGeojsonBlogListing(feature, map, getUserCoordinates());
        break;
      default:
