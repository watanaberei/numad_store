...
const testConnection = async () => {
  try {
    ...
    console.log('+++[testConnection] DB connection:', db.mongoose.connection.name);

    // Test store sync
    console.log('+++[testConnection] Starting store sync...');
    const syncResults = await syncAllStores();
    console.log('+++[testConnection] Sync complete. Results:', 
      syncResults.map(r => ({
        id: r?._id,
        slug: r?.slug,
        name: r?.name
    ...
    const storedCount = await StoreModel.countDocuments();
    console.log('+++[testConnection] Total stores in MongoDB:', storedCount);
...
const syncAllStores = async () => {
  try {
    debugLog('syncAllStores', 'Starting store sync');
    
    // 1. Get all stores from data.js
    const storeData = new StoreData();
    const allStores = await storeData.getAllStores();
    debugLog('syncAllStores', `Found ${allStores?.length} stores to sync`);

    // 2. Sync each store to MongoDB
    const results = await Promise.all(allStores.map(async (store) => {
      try {
        debugLog('syncAllStores', `Syncing store: ${store.slug}`);
        
        // Format store data for MongoDB
        const storeDoc = {
          name: store.headline?.text || store.title,
          storeId: store.sys?.id || store.slug,
          slug: store.slug,
          title: store.title || store.headline?.text,
          address: store.location?.address,
          streetNumber: store.location?.streetNumber,
          streetName: store.location?.streetName,
          state: store.location?.state,
          zip: store.location?.zip,
          city: store.location?.city,
          designator: store.location?.designator,
          storeCurrentStatus: store.storeCurrentStatus,
          storeRange: store.storeRange,
        ...
        console.log('+++[syncAllStores] Formatted store doc:', storeDoc);

        // Use existing upsertStore function
        ...
        console.log('+++[syncAllStores] MongoDB save result:', {
          id: result._id,
          slug: result.slug,
          name: result.name
        });
        ...
        errorLog('syncAllStores', `Error syncing store ${store.slug}: ${error}`);
        return null;
      }
    }));

    const successCount = results.filter(r => r).length;
    debugLog('syncAllStores', `Sync complete. ${successCount}/${allStores.length} stores synced`);
    
    return results;

  } catch (error) {
    errorLog('syncAllStores', error);
    throw error;
  }
};
