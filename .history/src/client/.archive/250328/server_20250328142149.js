...
app.get('/api/stores/current', async (req, res) => {
  try {
    ...
    console.log('[Current Store] Request URL:', request);
    
    if (!request.slug) {
      console.log('[Current Store] No slug found in URL');
      return res.status(404).json({ error: 'No store slug found' });
    }

    // Get store data from MongoDB
    const store = await Store.findOne({ slug: request.slug });
    
    if (!store) {
      console.log('[Current Store] Store not found for slug:', request.slug);
      return res.status(404).json({ error: 'Store not found' });
    }
