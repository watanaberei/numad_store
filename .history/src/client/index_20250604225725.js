...
// Helper function to ensure map container exists
const ensureMapContainer = () => {
  const mapContainer = document.getElementById('map-container');
  if (!mapContainer) {
    console.error('[ROUTER] Map container not found');
    return false;
  }
  return true;
};
...
const router = async () => {
  try {
    ...
    try {
      ...
      // Ensure map container exists before after_render
      if (parseUrl === "/" || parseUrl === "/stores/:slug") {
        if (!ensureMapContainer()) {
          console.error('[ROUTER] Map container not found, skipping map initialization');
        }
      }
      ...
      console.error('[ROUTER] Error during screen rendering:', e);
      const main = document.getElementById("content");
      main.innerHTML = await screen.render();
      if (screen.after_render) await screen.after_render();
    }
  } catch (error) {
    console.error('[ROUTER] Critical error in router:', error);
  }
};
