...
const storeCity = store.item[0].location.city;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (storeLocation && storeLocationData) {
    ...
    const matchedLocation = location.locations.find(loc => loc.city === storeCity)?.city === storeCity;
    // const matchedLocation = data.location?.find(loc => 
    //   loc.city === store?.item?.[0]?.location?.city
    // );

    console.log('debug log: location02 - Location matching:', {
      cityToMatch: store.city,
      matchedLocation
    });
