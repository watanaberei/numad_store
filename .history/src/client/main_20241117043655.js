...
document.addEventListener('DOMContentLoaded', () => {
  console.log('debug log: storeLocation01 - Full store data:', data.store);
  console.log('debug log: storeLocation02 - Store item location:', data.store?.item?.[0]?.location);
 ...
 if (storeHeroContainer && data.store?.hero?.[0]) {
  ...
  const storeLocationElement = document.getElementById('store-location');
  console.log('debug log: storeLocation02 - Element and data check:', {
    elementExists: !!storeLocationElement,
    elementId: storeLocationElement?.id,
    innerHTML: storeLocationElement?.innerHTML
  });

  if (storeLocationElement && data.store?.location?.[0]) {
    const locationData = data.store.location[0];
    console.log('debug log: storeLocation03 - Location data structure:', {
      header: locationData.header,
      attribute: locationData.attribute,
      text: locationData.text,
      footer: locationData.footer,
      cityFromStore: data.store?.item?.[0]?.location?.city,
      geotags: locationData.attribute?.geotags
    });
    ...
    try {
      ...
      console.error('debug log: storeLocation04 - Render error:', error);
    }
  }
});