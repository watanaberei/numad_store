...
export const attrtagScore = {
  ...
  afterRender: () => {
    console.log('debug11 log: attrtagscore afterRender ');
    const button = document.querySelectorAll('#attrtag-score');
      // attrtagButtons.forEach(button => {
        button.addEventListener('mouseenter', () => {
          console.log('attrtagscore mouseEnter CreateScoreInterface');
          button.classList.add('hovered');
          const tagSco  reResult = getStatsScore(button.getAttribute('data-score'));
          const Crea   teScoreInterface = {
            render: () => {
              return `
                <div class="score-icons">
                  ${tagScoreResult.icons.map((icon, index) => `
