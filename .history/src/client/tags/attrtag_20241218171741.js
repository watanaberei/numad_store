...
export const attrtagScore = {
  render: (data) => {
    ...
    return `
      ${tags.map(tag => {
        ...
        return `
          <button class="attrtag tag" id="attrtag-score" data-name="value" data-score="${tag.score}">
            <div class="single-icon">
              ${tagScoreResult.icons[tagScoreResult.currentScore]?.icon || ''}
            </div>
  ...
  afterRender: () => {
    document.addEventListener('DOMContentLoaded', () => {
      const attrtagButtons = document.querySelectorAll('#attrtag-score.attrtag.tag');
      console.log('debug log: storeLocation12 - Found Attrtag buttons:', attrtagButtons.length);
