...
const storeOverview = data.store?.item?.[0]?.overview?.[0];
const storeService = data.store?.item?.[0]?.service?.[0];
const storeExperience = data.store?.item?.[0]?.experience?.[0];
const storeHero = data.store?.item?.[0]?.hero?.[0];

// Extract store-specific data
const storeOverviewData = storeOverview;
const storeServiceData = storeService;
const storeExperienceData =storeExperience;
const storeLocationData = store?.location;
const storeHeroData = storeHero;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (storeHeroContainer && store?.hero) {
    console.log('debug log: hero01 - Rendering hero with:', store.hero);
    try {
      storeHeroContainer.innerHTML = components.storeHero.render(store.hero);
  ...
  if (storeLocationElement && store?.location) {
