...
const initializeData = data.initializeData;
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const dataSource = storeData?.hours || [];
  if (!dataSource.length) {
    console.error('No data available for timeline');
    return;
  }

    const businessHoursContainer = document.getElementById("business-hours");
    if (businessHoursContainer) {
      const timelineComponent = array.create.createTimeline(
        components.storeBusinessHours,
        storeData.hours,
        "timeline",
        "timeline-style",
        24
      ...
      businessHoursContainer.innerHTML = timelineComponent;
      array.create.initializeTimeline("business-hours");
    }
