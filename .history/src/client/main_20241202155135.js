...
console.log('debug log: location13 - Extracted data structures:', {storeLocationData});


// const city = location.locations.find(loc => loc.city === storeCity)?.city;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (businessHoursContainer && storeData?.hours) {
    console.log('Business hours data:', storeData.hours[0].open);
    businessHoursContainer.innerHTML = components.businessHours.render(storeData);
    components.businessHours.afterRender();
  }
  // Hero Section
  ...
  if (storeLocation && storeLocationData) {
    ...
    if (matchedLocation) {
      ...
      console.log('debug log: location15 - Rendering service with:', storeLocationHeader);
      console.log('debug log: service01 - Rendering service with:', storeServiceData);
      try {
