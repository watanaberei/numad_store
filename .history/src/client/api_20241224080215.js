...
export const getStore = async (storeId, limit = 100, skip = 0) => {
  try {
    ...
    const response = await client.getEntries({
      content_type: "store",
      ...(storeId ? { 'sys.id': storeId } : {}),
      limit,
      skip,
    });

    console.log('Raw store response:', response); // Debug log

    const stores = response.items.map(store => ({
      type: 'Feature',
      geometry: {
        type: 'Point',
        coordinates: [
          store.fields.location?.geolocation?.lon || 0,
          store.fields.location?.geolocation?.lat || 0
        ]
      },
      properties: {
        variant: 'stores', // Add this to match your switch case
        title: store.fields.title,
        slug: store.fields.slug,
        address: store.fields.location?.address,
        categoryType: store.fields.category?.categoryType,
        headline: store.fields.headline?.text,
        media: {
          thumbnail: store.fields.media?.thumbnail?.url,
          hero: store.fields.media?.hero?.url
        }
      }
    }));

    console.log('Transformed stores:', stores); // Debug log
    return stores;
  } catch (error) {
