...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (storeHero && store?.hero?.[0]) {
    console.log('debug log: hero01 - Rendering hero with:', store.hero?.[0]);
    try {
      storeHero.innerHTML = components.storeHero.render(store.hero?.[0]);
  ...
  if (storeLocation && storeLocationData) {
    ...
    console.log('debug log: location02 - Location matching:', {
      cityToMatch: store?.location?.locations?.city,
      matchedLocation: store?.item?.[0]?.location?.city,
    });
