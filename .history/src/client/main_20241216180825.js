...
const storeBusinessData = yelpApi.yelpApi.getStoreData(store.id);
console.log("=======", storeBusinessData);
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // Get data first
    const { storeBusinessData } = await data.initializeData();
    ...
    if (storeLocation && storeLocationData) {
      console.log("debug log: location01 - Location data:", {
        storeBusinessData: store,
        locationData: store.location,
        city: store?.location?.city
      });

      const storeBusinessData = store;
    ...
    if (storeBusiness && storeBusinessData?.hours) {
      console.log("Rendering business hours with data:", storeBusinessData.hours[0]);
      try {
        storeBusiness.innerHTML = components.storeBusiness.render(storeBusinessData);
