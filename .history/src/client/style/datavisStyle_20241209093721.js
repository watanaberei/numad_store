...
.datavis#time .datavis-item-details {
  display: none;
}
.datavis#time:hover .datavis-item-details {
  display: flex;
  flex-direction: column;
  alignItems: center;
  justifyContent: center;
}
.datavis#time .datavis-visual.active {
  background: var(--utility-rank-good03, #26e615);
}
.datavis#time .datavis-visual.inactive {
  background: #EAF2F4;
}
.datavis#time.current {
  border: 2px solid var(--utility-rank-good03);
  order: -1;
}
.datavis#time .time-item-current {
...
.datavis#time .time-hour {
  flex-shrink: 0;
  width: 7px;
  height: 9px;
  position: static;
}
.datavis#time .time-meridian {
  display: flex;
  flex-direction: row;
  gap: 10px;
  align-items: center;
