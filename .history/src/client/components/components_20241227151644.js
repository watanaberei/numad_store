...
export const storeLocation = {
  ...
  afterRender: (data) => {
    ...
    const coordinates = {
      lat: data?.coordinates?.latitude || data?.geolocation?.lat,
      lon: data?.coordinates?.longitude || data?.geolocation?.lon
    };
...
//   render: (attribute) => {
//     console.log('mapNearby render with attribute:', attribute);
//     return `
...
//     `;
//   },
//   afterRender: (coordinates) => {
//     console.log('debug log: mapNearby01 - Running afterRender with coordinates:', coordinates);
//     const mapElement = document.getElementById('map');
//     if (mapElement) {
//       try {
//         console.log('debug log: mapNearby02 - Found map element, initializing map');
//         initMap({
//           container: 'map', // Use 'map' instead of 'map-container'
//           style: 'mapbox://styles/mapbox/streets-v11',
//           center: [coordinates?.lon || -118.2437, coordinates?.lat || 34.0522],
//           zoom: 13,
...
export const mapNearby = {
  render: (data) => {
    ...
    return `
      <div class="location col04">
        <div class="map-container col04">
          <div id="map" class="map col04"></div>
        </div>
        <div class="sidebar col04">
          <div id="listings" class="listings grid08-overflow span04 row01"></div>
        </div>
  ...
  afterRender: (coordinates) => {
    ...
    if (mapElement && coordinates) {
      try {
        console.log('debug log: mapNearby02 - Initializing map');
        initMap({
          container: 'map',
          style: 'mapbox://styles/mapbox/streets-v11',
