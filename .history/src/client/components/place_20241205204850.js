...
export const businessHours = {
    render: (schedule) => {
      const hoursData = Array.from({length: 24}, (_, i) => ({
        hour: i,
        status: null // Will be populated with actual status data
      }));
  
      return array.create.createTimeline(
        {
          render: (hourSlot) => `
            <div class="hour-slot ${hourSlot.status || ''}" data-hour="${hourSlot.hour}">
              <div class="hour-marker">
                <span class="text02">${hourSlot.hour}:00</span>
              </div>
              <div class="status-info">
                <div class="hover-content">
                  <span class="text02">Hour: ${hourSlot.hour}:00</span>
                  <span class="text02">Status: ${hourSlot.status || 'Unavailable'}</span>
                </div>
              </div>
            </div>
          `
