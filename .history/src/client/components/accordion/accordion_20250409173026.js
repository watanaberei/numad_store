...
export const Details = {
  render: (data) => {
    ...
    sections.forEach((section, index) => {
      const { title, subtitle, content, isOpen = false } = section;
      
      accordionHTML += `
        <div class="accordion-section" data-index="${index}">
          <div class="title">
            <button class="accordion-header ${section.isOpen ? 'active' : ''}" aria-expanded="${section.isOpen}">
              <span class="label" data-class="text02">${section.title}</span>
              <span class="label" data-class="text02">${section.subtitle}</span>
              <div class="button2">
          ...
          <div class="accordion-content" ${section.isOpen ? '' : 'hidden'}>
            ${section.content}
          </div>
