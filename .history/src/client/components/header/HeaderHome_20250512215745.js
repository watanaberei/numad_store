...
const HeaderHome = {
  render: async () => {
    try {
      ...
      // // console.log("Rendering HeaderHome with auth status:", isAuthenticated);
      ...
      <nav id="HeaderHome" class="nav navigation container nav-top">
        <section class="grid05 base nav-main">
              ...
              <div class="col01 nav-main-left left">
                ...
                <div class="col01 nav-main-right right">
                  <div class="nav-main-right-container">
                    ${isAuthenticated ? `
                      <button id="btn-user" class="btn-nav text03">${userEmail}</button>
                      <button id="btn-logout" class="btn-nav text03">Log out</button>
                    ` : `
                      <button id="btn-login" class="btn-nav text03  ">Login</button>
                    `}
                  </div>
                </div>
              </section>
    ...
    -->
      ...
      console.error('Error in HeaderHome render:', error);
  ...
  after_render: async (map) => {
    try {
      ...
      // Get the modal components from modals.js
      const { modalAuth: modalAuthHTML, after_render: modalAfterRender } = await modals.modalAuth();
      ...
      // Important: Make sure the modal is in the DOM
      let modal = document.getElementById('myModalAuth');
      if (!modal) {
        // If modal doesn't exist, add it to the DOM
        document.body.insertAdjacentHTML('beforeend', modalAuthHTML);
        modal = document.getElementById('myModalAuth');
        if (modalAfterRender) modalAfterRender();
      }
      ...
      if (isAuthenticated) {
        ...
        document.getElementById('btn-logout').addEventListener('click', function() {
          ...
          window.location.href = '/'; // Reload page to update UI
        });
      } else {
        ...
        //     modalAccount();
        //   });
         // Login button handler - make sure it shows the modal
      ...
      if (isAuthenticated) {
        ...
        document.getElementById('btn-login').addEventListener('click', function() {
          // Always show the modal using the .show class
          modal.classList.add('show');
          // Optionally reset modal content/state here if needed
          if (modalAfterRender) modalAfterRender();
        });
        // Also allow modal to be closed by clicking the close button or background
      ...
      console.error('Error in HeaderHome after_render:', error);
    }
  },
