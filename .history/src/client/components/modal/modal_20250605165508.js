///////////////////////// START FIXED MODAL AUTHENTICATION /////////////////////////
// src/client/components/modal.js - FIXED VERSION with better error handling
...
async function verifyTokenOnce(token) {
  ...
  try {
    ...
    const response = await fetch('http://localhost:4500/verify-token', {
      method: 'POST',
      headers: {
...
async function refreshAccessToken() {
  ...
  try {
    ...
    const response = await fetch('http://localhost:4500/refresh-token', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function handleFormSubmit(e) {
  ...
  try {
    ...
    if (authState.retryCount < authState.maxRetries && (error.message.includes('timeout') || error.message.includes('Server error') || error.message.includes('Network'))) {
...
// FIXED: Continue with the rest of the modal functions...
// (The remaining functions would follow the same pattern with improved error handling)
