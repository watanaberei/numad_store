    ...
    modalBtns.forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        if (target === 'auth-modal') {
          ...
          modalElements.openAuth('check');
        } else {
          // Regular modal handling for other modalElements
    ...
    return {
      ...
      openAuth: (type = 'check') => {
        ...
        if (authModal) {
          ...
          authLog('Auth modal content rendered');
        } else {
          authLog('Failed to find or create auth modal');
        }
      }
    };
  }
...
export const auth = {
  ...
  openAuthModal: (type = 'login') => {
    modalElements.init().openAuth(type);
  }
};
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    authLog('DOM loaded, initializing modalElements');
    const modalSystem = modalElements.init();
    authLog('Modal system initialized', { modalSystem });
  });
} else {
  authLog('DOM already loaded, initializing modalElements');
  const modalSystem = modalElements.init();
  authLog('Modal system initialized', { modalSystem });
}