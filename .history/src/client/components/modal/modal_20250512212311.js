   ...
   const after_render = async () => {
        ...
        if (emailInput) {
          ...
          try {
            ...
            if (data.errors) {
              ...
              if (data.user) {
                console.log('[modal] Rendering login form for user:', email);
                authForm.innerHTML = `
                <!--
                <form id="auth-form">
                  <input type="email" id="email" name="email" placeholder="Email" required>
                ...
                // Always re-select the new form and add a submit event listener
                const loginForm = document.getElementById('login-form');
                if (loginForm) {
                  loginForm.addEventListener('check', async (event) => {
                    ...
                    if (passwordInput) {
                      ...
                      try {
                        ...
                        // Store tokens and close modal on success
    ...
    document.addEventListener('click', (event) => {
      ...
      if (event.target.classList.contains('modal-close') || event.target.classList.contains('modal')) {
        if (authModal) {
          authModal.classList.remove('show');
          console.log('[modal] Modal closed by close button or background click');
        }
      } else if (event.target.matches('#auth-btn')) {
        if (authModal) {
          authModal.classList.remove('hide');
          authModal.classList.add('show');
          console.log('[modal] Modal opened by auth-btn');
        }
      }
    });
  };
