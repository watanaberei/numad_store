...
export const amtagCategory = {
  render: (data) => {
    return `
    <button class="amtag tag amtagCategory" id="amtag" data-name="value" ${
      data.url ? `data-url="${data.url}"` : ""
    }>
...
export const amtagScore = {
  ...
  afterRender: () => {
    ...
    // Existing amtag button handlers
    const amtagButtons = document.querySelectorAll('#score.amtag.tag');
    console.log('debug log: storeLocation12 - Found amtag buttons:', amtagButtons.length);
