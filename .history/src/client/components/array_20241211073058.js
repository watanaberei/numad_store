...
const timelineControls = controls.timelineControl;
...
export const create = {
  ...
  createTimeline: (component, dataSource, type, style, limit) => {
    const processedData = dataSource.slice(0, limit).map((item, index) => ({
      ...item,
      index,
    }));
    const currentHour = new Date().getHours();
    const timelineItems = dataSource.map((item, index) => {
      const isActive = item.status === "active";
      const isCurrent = item.isCurrent;

      return `
        <div class="datavis-item col01 ${isActive ? "active" : "inactive"} ${isCurrent ? "current" : ""}">
          <div class="datavis-visual">
            ...
            <div class="datavis-info">
              <div class="primary">
                <div class="pill">
                  <div class="content">
                    <span class="label text02">${item.day}</span>
                  </div>
                </div>
              </div>
              <span class="secondary sentance">
                <i class="icon-indicator-live"></i>
                <span class="time word">
                  <span id="time-hour">${item.startHour}</span>
                  <span id="time-meridian">${item.startMeridian}</span>
                </span>
              </span>
            </div>
          </div>
    ...
    // Return full timeline component
    return `
      <div class="business-hours col04">
        <div class="container col04">
          <div class="datavis grid08-overflow">
            ${timelineItems}
          </div>
        </div>
        <div class="tools col04">
          <div class="detail col03">
            <div class="status">
  ...
  initializeTimeline: (containerId, dataSource) => {
    ...
    // Render timeline into the container
    container.innerHTML = create.createTimeline(dataSource);
    ...
    if (!container) {
      console.error(`Timeline container not found: ${timelineType}`);
      return;
    }
    ...
    prevButton.addEventListener("click", () => {
      scrollPosition = Math.max(0, scrollPosition - 200); // Adjust based on your timeline width
      timeline.scrollTo({ left: scrollPosition, behavior: "smooth" });
    });

    nextButton.addEventListener("click", () => {
      scrollPosition = Math.min(timeline.scrollWidth, scrollPosition + 200);
      timeline.scrollTo({ left: scrollPosition, behavior: "smooth" });
    });
    controls.timelineControl.initialize(timelineType);

    
  // initializeTimeline: (containerId) => {
  //   const container = document.getElementById(containerId);
  //   if (!container) {
  //     console.error(`Timeline container not found: ${containerId}`);
  //     return;
  ...
  //   controls.timelineControl.initialize(container);
  // },
  ...
  const slots = timeline.querySelectorAll('.datavis-item');
  ...
  const scrollToCurrentHour = () => {
    ...
    if (currentSlot) {
      ...
      timelineControls.updateTimelineView(Math.max(0, currentHour - 2)); // Center current hour
      
      // Update current time indicator
      timelineControls.updateCurrentTimeMarker();
    }
  };
