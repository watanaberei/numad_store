...
export const create = {
  ...
  if (type === 'business-hours') {
    // Pass data directly to the business hours component
    return component.render(dataSource);
  }
    console.log('Creating timeline:', { type, sortBy, style, dataSource });
    ...
    if (type === 'business-hours') {
      console.log('Rendering business hours timeline:', {
        currentHour,
        itemsCount: processedData.length
      });
      
      // Let the places component handle its own rendering
      return component.render(processedData);
    }

    // Default timeline rendering for other types
    return `
      <div class="timeline-container col04 ${style}" data-timeline-type="${type}">
        ...
        <div class="timeline-track grid04-overflow">
          ${processedData.map(item => `
            <div class="timeline-slot col01 ${item.hour === currentHour ? 'current' : ''}"
                 data-hour="${item.hour}"
  ...
  initializeTimeline: (timelineType) => {
    ...
    if (timelineType === 'business-hours') {
      // Initialize business hours specific controls
      const container = timeline.closest('.business-hours');
      if (container) {
        console.log('Initializing business hours controls');
        return timelineControls.initializeBusinessHours(container);
      }
    }

    // Initialize standard timeline controls
