...
import * as yelpApi from '../../../server/data/yelp/yelpApi.js'; // Add this import
import * as control from '../controls/controls.js'; // Add this import
import * as place from '../datavis/datavisTimeline.js'; // Add this import


// const timelineControls = control;
...
export const create = {
      ...
      contents.forEach(content => {
        ...
        if (areaPill) {
          areaPill.addEventListener('click', (e) => {
            ...
            const areaData = StoreData.area.item.find(i => i.name === area);
            const galleryLink = areaData?.links?.gallery;
            if (galleryLink) {
        ...
        if (categoryPill) {
          categoryPill.addEventListener('click', (e) => {
            ...
            const categoryData = StoreData.serviceCategoryData[category.toLowerCase()];
  ...
  createTimeline: (component, dataSource, type, sortBy = 'chronological', style, limit) => {
    ...
    <div class="datavis business-hours col04 ${style}" data-timeline-type="${type}">
      ...
      <div class="container timeline-track grid08-overflow">
        ${processedData.map(item => `
          <div class="datavis-item col01 ${item.hour === currentHour ? 'current' : ''}" 
               ...
               <div class="datavis-visual ${item.status}">
                    <div class="datavis-container ${item.status}">
                    ...
                    <div class="secondary">
                        <div class="time">
                            <span class="text02 word">
                                <span id="time-hour" class="text02">${item.hour}</span>
                                <span id="time-meridian" class="text02">${item.info.meridian}</span>
                            </span>
                        </div>
                    </div>
                </div>
...
initializeTimeline: (timelineType) => {
  ...
  const controls = control.controlTimeline.initializeDatavisControl(timeline, {
