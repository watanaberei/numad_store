...
import * as places from "./timeline/timelineTimeline.js";
...
export const mapNearby = {
  ...
  afterRender: (data) => {
    console.log('-06: debug log: mapNearby01 - Running afterRender');
    const mapElement = document.getElementById('map');
    const coordinates = data.geolocation.coordinates;
    if (mapElement) {
      console.log('-07: debug log: mapNearby02 - Found map element');
      const mapData = {
        container: 'map',
    ...
    mapElement.addEventListener("load", async () => {
      ...
      store: data
        ...
        console.log("Map initialized successfully", mapData);
      } catch (error) {
        console.error("Error initializing map:", error);
      }
    });
    } else {
      console.warn('-08:Map element not found');
    }
  }
};
