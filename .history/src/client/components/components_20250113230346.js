...
export const storeHero = {
  render: async (hero) => {
    ...
    try {
      // Wait for all components to resolve
      const [detailHTML, galleryHTML, headlineHTML] = await Promise.all([
        storeDetail.render(hero),
        heroGallery.render(hero),
        storeHeadline.render(hero)
      ]);
      ...
      console.error("Error rendering storeHero:", error);
...
export const storeService = {
 render: (headerData, textData, categoryData, footerData) => {     
   console.log('storeService render with categoryData:', categoryData);
   ...
   return `
     ...
     <div class="col04 container body">
       <div class="col04 array content">
        ...
        //    return storeCategory.render(categoryData);
        //  })()}
        ${categoryData ? storeCategory.render(categoryData) : ''}
       </div>
       ${dividerComponent.render()}
       <div class="col04" id="text">
         ${textData ? textBlock.render(textData) : ''}
...
export const storeBusiness = {
  render: (headerData, timelineData, footerData) => {
    return `
      <div class="col04 array" id="header">
        ${headerData ? sectionHeader.render(headerData) : ''}
      </div>
      <div class="grid04-overflow array" id="business-hours">
        ${timelineData ? storeBusinessTimeline.render(timelineData) : ''}
      </div>
      <div class="col04" id="footer">
        ${footerData ? sectionFooter.render(footerData) : ''}
      </div>
...
export const storeLocation = {
  render: (headerData, attributeData, footerData) => {
    console.log('storeLocation render with data:', { headerData, attributeData, footerData });
    return `
      <div class="col04 array" id="header">
        ${headerData ? sectionHeader.render(headerData) : ''}
      </div>
      ...
      <div class="col04 container body">
        <div class="col04">
          ${mapNearby.render(attributeData)}
        </div>
        ${dividerComponent.render()}
        <div class="col04" id="attributes">
          ${attributeData ? locationAttributes.render(attributeData) : ''}
        </div>
      </div>
    ...
    return `
      <div class="hero-detail array col04">
        <div class="hero-primary tag-array">
          ${geotag.geotagRating.render({ rating: heroData.rating })}
          ${geotag.geotagCostEstimate.render({ priceRange: heroData.price })}            
        </div>
        <div class="hero-secondary tag-array">
          <span class="objtag info-type text02">${heroData.storeType || ''}</span>
          <span class="objtag info-distance text02">${heroData.distance || '0'} away in</span>
          <span class="objtag info-city text02">${heroData.city || ''}, ${heroData.state || ''}</span>
        </div>
      </div>
...
export const heroGallery = {
  render: async (data) => {
    ...
    if (!heroData?.photos || !heroData.photos.length) {
    ...
    return `
      <div class="hero-gallery00 grid05-overflow">
        ${heroData.photos.slice(0, 4).map((image, index) => `
          <div
  ...
  render: async (data) => {
    ...
    return `
      <span class="hero-title array span04">
        <span class="store-name text05">${heroData.name || 'Store Name Unavailable'}</span>
        <span class="store-city text05">${heroData.location?.city || ''}</span>
      </span>
      <div class="hero-distance word col01">
        <span class="glyph05 glyph-location-pin">${glyph.glyphLocationPin}</span>
        <span class="distance-value text05">${heroData.distance || '0'}</span>
        <span class="distance-unit text05">mi</span>
      </div>
      ${geotag.geotagStatus.render({ status: heroData.hours?.[0]?.is_open_now ? 'Open' : 'Closed' })}
    `;
  }
};
