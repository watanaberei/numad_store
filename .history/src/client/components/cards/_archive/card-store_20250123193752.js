...
const createStoreCard = {
  ...
  `;
   ...
   const infoTagCityData = {
    ...
    console.log("*00:storeData",storeData)

    // After a delay, replace the loader with the actual content
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve(`
           <div class="card col01">
            <div class="top primary">
              <!--$ {amtag.amtagGroup.render({ category: storeData.environment })}-->
              ${amtag.amtagItem.render({ category: storeData.rating })}
              ${amtag.amtagItem.render({ category: storeData.environment })}              
            </div>
            <div class="middle tertiary">
            </div>
            <div class="bottom secondary">
              <img src="${galleryURL}" alt="alt" />
              <div class="item-info">
                <span class="name text02">${storeData.title}</span>
                <span class="type text02">${storeData.distance}</span>
              </div>
            </div>
          </div>
         ...
         $ {
          storeData?.logo
            ? ``
          <div class="badge">
            <img src="$ {storeData.logo}" alt="Partner logo" class="partner-logo-thumbnail" width="15" height="15">
          </div>
        ``
            : ""
        }
