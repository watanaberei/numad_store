  ...
  `;
   ...
   const infoTagCityData = {
    ...
    async function getStoreRange() {
        try {
            getStoreRange.storeRange = await storeData.storeRange; // assuming storeRange is a property of storeData
            // console.log(getStoreRange.storeRange); // should print '18.94' instead of '[object Promise]'
            return getStoreRange.storeRange; // return the value
        } catch (error) {
            console.error('An error occurred:', error);
        }
    }
    console.log("getStoreRange in card-store:", getStoreRange);
    ...
    const storeRange = getStoreRange.storeRange || '';
    // const renderedEyebrow = eyebrow.render(getStoreRange.storeCurrentStatus, getStoreRange.storeRange);
