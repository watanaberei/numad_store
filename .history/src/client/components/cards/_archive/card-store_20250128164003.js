...
const createStoreCard = {
  ...
  `;
   ...
   const infoTagCityData = {
    ...
    const stattagRange = geotag.statTag.render(storerange);
    const stattagHour = geotag.statTagLg.render(statTagHourData);
    const mediaThumbnail = media.thumbnail.render(gallery, galleryLimit);
    const tagNeustar = tag.tagNeustar.render(neustar);
    const tagRating = geotag.geotagRating.render(rating);
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve(`
          ...
          <!--$ {cardEyebrow}-->
          <div class="background media thumbnail loading">
              ${mediaThumbnail} 
