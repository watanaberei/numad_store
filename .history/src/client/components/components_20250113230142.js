    ...
    return `
      <div class="hero-detail array col04">
        <div class="hero-primary tag-array">
          ...
          ${geotag.geotagCostEstimate.render({ priceRange: heroData.costEstimate })}            
        </div>
        <div class="hero-secondary tag-array">
          <span class="objtag info-type text02">${heroData.storeType}</span>
          <span class="objtag info-distance text02">${heroData.distance} away in</span>
...
export const heroGallery = {
  render: async (data) => {
    ...
    if (!heroData?.gallery || !heroData.gallery.length) {
    ...
    return `
      <div class="hero-gallery00 grid05-overflow">
        ${heroData.gallery.slice(0, 4).map((image, index) => `
          <div
  ...
  render: async (data) => {
    ...
    return `
      <span class="hero-title array span04">
        <span class="store-name text05">${heroData.storeName}</span>
        <span class="store-city text05">${heroData.city}</span>
      </span>
      <div class="hero-distance word col01">
        <span class="glyph05 glyph-location-pin">${glyph.glyphLocationPin}</span>
        <span class="distance-value text05">${heroData.distanceMiles}</span>
        <span class="distance-unit text05">mi</span>
      </div>
      ${geotag.geotagStatus.render({ status: heroData.status })}
    `;
  }
};
