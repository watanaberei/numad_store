...
import * as MapRoute from "../mapRoute.js";
...
export function createGeojsonStoreListing(store, map, userCoordinates) {
  console.log("~05:Creating store listing with:", store);
  console.log("~06:Store gallery data:", store.properties?.gallery);
  ...
  const galleryData = originalGallery;
  console.log("~07:Processed gallery data:", galleryData);
  ...
  // listing.className = " " + "listing" + " " + "grid03" + " " + "col03" + " " + "store" + " " + "card-listing" + " " + "col03";
  ...
  carousel.className = "card" + " " + "store" + " " + "col01";
  ...
  cardContainer.className = "card" + " " + "store" + " " + "col01" + " " + "item";
  cardContainer.href = "/" + variant + "/" + slug;
  cardContainer.rel = "noopener noreferrer nofollow";
  cardContainer.target = categoryType + "-${store.sys.id}";
  ...
  if (variant === "stores") {
    ...
    .then(storeContent => {
      ...
      // //console.log("*00:storeContent", storeContent);
      // const storeCarouselItem = components.storeCardThumbnail.render(storeContent);
...
function generateCarouselItem(content) {
  ...
  carouselItem.className = "container gridCardgalleryItem";
...
function generateGalleryHTML(gallery) {
  console.log("~08:Generating gallery HTML with:", gallery);
  
  if (!Array.isArray(gallery)) {
    console.warn("Gallery is not an array:", gallery);
    return '';
  }

  let galleryHTML = '<div class="media-container carousel">';
  
  gallery.slice(0, 3).forEach((galleryItem, index) => {
    // Handle both string URLs and object formats
    const imageUrl = typeof galleryItem === 'string' ? galleryItem : galleryItem.url;
    if (!imageUrl) {
      console.warn("Invalid gallery item:", galleryItem);
      return;
    }
    ...
    galleryHTML += `
      <div class="carousel-item">
        <img src="${imageUrl}" 
             id="image${index.toString().padStart(2, '0')}" 
             class="image ratio1x1" 
