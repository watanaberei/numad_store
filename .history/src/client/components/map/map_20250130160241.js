...
export async function initMap(mapData) {
  try {
    ...
    if (!store) {
      console.warn("[initMap] No store data available");
      return null;
    }

    const nearbySearchParams = {
      storeAddress: store.location?.address || null,
      storeCity: store.location?.city || null,
      storeState: store.location?.state || null,
      storeLongitude: store.coordinates?.longitude || null,
      storeLatitude: store.coordinates?.latitude || null
    };
    ...
    // Only update features if we have valid data
    if (mapData?.stores) {
    ...
    console.log("nearbyStoreData", nearbyStoreData || null);
  } catch (error) {
