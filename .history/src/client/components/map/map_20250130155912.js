...
export async function initMap(mapData) {
  try {
    console.log("[initMap] Starting with data:", mapData);
    
    // Validate input
    if (!mapData?.data) {
      console.warn("[initMap] No map data available");
      return null;
    }

    const store = mapData.data;
    
    // Use contentful data if yelp data is not available
    const storeLocation = {
      address: store.location?.address || store.storeAttributes?.find(attr => attr.key === 'Address')?.value,
      city: store.location?.city || store.storeAttributes?.find(attr => attr.key === 'City')?.value,
      state: store.location?.state || store.storeAttributes?.find(attr => attr.key === 'State')?.value,
      longitude: store.coordinates?.longitude || store.storeLongitude,
      latitude: store.coordinates?.latitude || store.storeLatitude
    };

    if (!storeLocation.longitude || !storeLocation.latitude) {
      console.warn("[initMap] Missing coordinates");
      return null;
    }

    const nearbySearchParams = {
      storeAddress: storeLocation.address,
      storeCity: storeLocation.city,
      storeState: storeLocation.state,
      storeLongitude: storeLocation.longitude,
      storeLatitude: storeLocation.latitude
    };

    console.log("[initMap] Search params:", nearbySearchParams);
