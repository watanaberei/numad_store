...
export const mapNearby = {
  ...
  afterRender: async () => {
    ...
    const mapContainer = document.getElementById('map-container');
      if (mapContainer) {
        console.log("debug log: map01 - Found map container, initializing map");
        try {
          await new Promise(resolve => setTimeout(resolve, 100)); // Small delay to ensure DOM is ready
          initMap({
            container: 'map-container',
            style: 'mapbox://styles/mapbox/streets-v11',
            center: [storeLocationData.coordinates.lon, storeLocationData.coordinates.lat],
            zoom: 13
          });
        } catch (error) {
        ...
        console.warn("Map container not found in after_render");
      }
    } catch (error) {
