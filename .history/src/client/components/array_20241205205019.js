...
export const create = {
  ...
  createTimeline: (component, dataSource, type, sourceType, style, limit) => {
    console.log('Creating timeline:', { type, sourceType, style });
    
    const processedData = dataSource.slice(0, limit);
    
    return `
      <div class="timeline-container col04 ${style}" data-timeline-type="${type}">
        <div class="timeline-track grid24-overflow">
          ${processedData.map((item, index) => `
            <div class="timeline-slot col01" data-index="${index}">
    ...
    const slots = timeline.querySelectorAll('.hour-slot');
    ...
    slots.forEach(slot => {
      slot.addEventListener('mouseenter', () => {
        const info = slot.querySelector('.status-info');
