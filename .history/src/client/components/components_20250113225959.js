    ...
    return `
      <div class="store-detail">
        <div class="meta">
          ...
          <span class="cost-estimate">${data.costEstimate || ''}</span>
        </div>
        <div class="location">
          <span class="distance">${data.distance || ''}</span>
          <span class="city-state">${data.city || ''}, ${data.state || ''}</span>
        </div>
...
export const heroGallery = {
  render: (data) => {
    ...
    if (!data?.gallery || !Array.isArray(data.gallery)) {
      console.warn("[heroGallery] No gallery data provided");
      return '';
    }
  ...
  render: (data) => {
    ...
    return `
      <div class="store-headline">
        <h1 class="store-name">${data.storeName || 'Store Name Unavailable'}</h1>
        <div class="store-types">
          ${(data.storeType || []).map(type => `
            <span class="store-type">${type}</span>
          `).join('')}
        </div>
      </div>
    `;
