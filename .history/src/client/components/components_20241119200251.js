...
export const locationAttributes = {
  render: (data) => {
    ...
    const visibleSections = data.section.slice(0, 3);
    const hiddenSectionsCount = Math.max(0, data.section.length - 3);

    return `
      <div class="location-attributes-container">
...
//             </div>
//             <div class="tag-array">
//               ${geotag.attrtagAttribute.render({
//                 tags,
//                 limit: 2
//               })}
//             </div>
...
export const geotagAttributes = {
  render: (data) => {
    const attrtagAttribute = attrtag.attrtagScore;
    const amtagScore = "s";
    return `
    <div class="array">
      ${attrtagAttribute}
      <div class="tag-array">
        ${data.map(item => attrtagAttribute.render(item)).join('')}
      </div>
    </div>
    `;
  }
...
export const locationTitle ={
  render: (data) => {
    const geotagAttribute = geotag.attrtagAttribute;
    const area = data.area;
    const city = data.city;
    return `
      <div class="tag-array">
