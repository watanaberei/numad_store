...
export const create = {
  ...
  createTimeline: (component, dataSource, type, sortBy = 'chronological', style, limit) => {
    console.log('Creating timeline:', { type, sortBy, style });
      ...
      <div class="timeline-container col04 ${style}" data-timeline-type="${type}">
        <div class="timeline-track grid04-overflow">
          ${processedData.map(item => `
            <div class="timeline-slot col01" data-index="${item.index}">
              ${component.render(item)}
            </div>
          `).join('')}
        </div>
  ...
  initializeTimeline: (timelineType) => {
    ...
    const showHourDetails = (slot) => {
      const hour = slot.dataset.index;
      const info = slot.querySelector('.hour-info');
      if (info) {
    ...
    slots.forEach(slot => {
      slot.addEventListener('mouseenter', () => showHourDetails(slot));
      slot.addEventListener('mouseleave', () => hideHourDetails(slot));
    });
