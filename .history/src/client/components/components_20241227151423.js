...
// Update the mapNearby component
export const mapNearby = {
  ...
  afterRender: () => {
    ...
    if (mapElement) {
      try {
        const storeLocation = {
          lat: data.store?.item?.[0]?.location?.coordinates?.latitude || 0,
          lon: data.store?.item?.[0]?.location?.coordinates?.longitude || 0
        };

        initMap({
          container: "map-container",
          style: "mapbox://styles/mapbox/streets-v11",
          center: [storeLocation.lon, storeLocation.lat],
          zoom: 13,
