...
import { initM// Main data access constants at the top of main.js
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (storeLocationElement && store?.location) {
    ...
    console.log('debug log: location01 - Location matching:', {
      storeCity,
      allLocations: data.location?.locations
    });

    const matchedLocation = data.location?.locations?.find(loc => loc.city === storeCity);
    
    if (matchedLocation) {
      console.log('debug log: location02 - Matched location data:', matchedLocation);
