...
const StoreScreen = {
  render: async () => {
    try {
      ...
      if (!slug) {
        console.error("[StoreScreen] No slug found in URL");
      ...
      try {
        ...
        console.log("[StoreScreen] Store initialized:", initResult);

        // Then get the current store data
        const currentStoreResponse = await fetch(`http://localhost:6000/api/stores/current?slug=${slug}`, {
          headers: {
            'Content-Type': 'application/json'
          }
        });
