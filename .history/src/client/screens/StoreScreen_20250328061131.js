...
const StoreScreen = {
  render: async () => {
    try {
      ...
      // 4. Save to MongoDB by calling the API endpoint
      try {
        const response = await fetch(`http://localhost:4000/api/stores/sync/${request.slug}`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          console.log("[StoreScreen] Store data synced with MongoDB");
        } else {
          console.error("[StoreScreen] Failed to sync store data with MongoDB");
        }
      } catch (syncError) {
        console.error("[StoreScreen] Error syncing with MongoDB:", syncError);
        // Continue rendering even if sync fails
      }

      // 5. Extract and format data for components
      const {
        hero = {},
        overview = {},
        service = {},
