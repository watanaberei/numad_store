...
const StoreScreen = {
  render: async () => {
    ...
    try {
      ...
      const yelpData = await storeData.getStoreBySlug(request.slug);
      console.log("%%%%[StoreScreen.render] Yelp data:", yelpData);

      // 3. Create store object using store() method
      const store = storeData.store(yelpData);
      console.log("%%%%[StoreScreen.render] Store object:", store);

      if (!store) {
        return `<div>Store not found</div>`;
      }

      // 4. Extract all the required data sections
      const storeHeroData = store.hero || {};
      const storeOverviewData = store.overview || {};
      const storeServiceData = store.service || {};
      const storeExperienceData = store.experience || {};
      ...
      const storeDetailData = store.hero || {}; // Using hero data for details
      ...
      // 6. Return the template with all sections
      return `
