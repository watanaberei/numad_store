...
const HomeScreen = {
  render: async () => {
    return `
      ...
      <container class="col02 secondary">
        <div class="content">
          <div class="title">
            <span class="text03">
              secondary
...
function renderFeatures(features, map) {
  ...
  features.forEach((feature) => {
    ...
    const gallery = feature.properties?.storeCardData?.gallery;
    if (!gallery || !Array.isArray(gallery)) {
      console.warn('Feature missing or invalid gallery data:', feature);
      return;
    }
    ...
    try {
      switch (feature.properties.variant) {
        case "stores":
          element = createGeojsonStoreListing(feature, map);
          break;
        case "reviews":
