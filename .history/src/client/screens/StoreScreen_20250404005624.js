    ...
    try {
      ...
      if (!store) {
        ...
        try {
          ...
          const mongoResponse = await fetch(`http://localhost:5000/api/stores/${request.slug}`);
      ...
      try {
        ...
        //  const response = await fetch(`http://localhost:5000/api/stores/${request.slug}`, {
        // const response = await fetch(`http://localhost:5000/api/stores/sync/${request.slug}`, {
        ...
        //   const retryResponse = await fetch(`http://localhost:5000/api/stores/${request.slug}`, {
        //     method: 'POST',
        //     headers: {
        //       'Content-Type': 'application/json'
        //     },
        ...
        if (syncError.message.includes("Failed to fetch")) {
          console.error("[StoreScreen] Connection error - make sure server is running on port 5000");
          console.error("[StoreScreen] Server URL:", `http://localhost:5000/api/stores/sync/${request.slug}`);
        }
...
window.storeActions = {
  ...
  toggleImpression: async function (storeId, type) {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/impression", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const saveStoreData = async (storeData) => {
  try {
    ...
    // const response = await fetch(`http://localhost:5000/api/stores/sync/${storeData.slug}`, {      
    const response = await fetch(`http://localhost:5000/api/stores/sync/${storeData.slug}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
