...
import { makeServerRequest } from "../../server/utils/utils.server.js";
...
const StoreScreen = {
  render: async () => {
    try {
      ...
      // Post store slug to server using makeServerRequest
      try {
        const response = await fetch(`${API_URL}/api/storAQ!@es/current`, {
          method: 'POST',
          headers: {
      ...
      if (accessToken) {
        try {
          ...
          // const result = await response.json();
          const result = await makeServerRequest(
            `${API_URL}/api/init-store/${request.slug}`,
            'POST',
            null,
            {
              headers: {
