...
// Keep socket.io on port 4000 for auth/real-time updates
const socket = io("http://localhost:4000");

// But ensure store data requests go to port 6000
...
const StoreScreen = {
  render: async () => {
    try {
      ...
      if (accessToken) {
        try {
          console.log('[StoreScreen] Recording store visit...');
          const visitResponse = await fetch(`http://localhost:6000/api/stores/visit/${request.slug}`, {
            method: 'POST',
            headers: {
          ...
          if (!visitResponse.ok) {
            ...
            console.error('[StoreScreen] Visit recording failed:', errorData);
            throw new Error(`Failed to record visit: ${visitResponse.statusText}`);
          }

          const visitResult = await visitResponse.json();
          console.log('[StoreScreen] Visit recorded successfully:', visitResult);
        } catch (error) {
          console.error('[StoreScreen] Visit recording error:', error);
          // Don't throw here - we want to continue loading the page even if visit recording fails
        }
      }

      // Fetch store data
      console.log('[StoreScreen] Fetching store data...');
      const storeResponse = await fetch(`http://localhost:6000/api/stores/${request.slug}`);
      if (!storeResponse.ok) {
        throw new Error(`Failed to fetch store: ${storeResponse.statusText}`);
      }
      const store = await storeResponse.json();
      console.log("[StoreScreen] Store data received:", store);
  ...
  after_render: async () => {
    try {
      ...
      if (accessToken) {
        try {
          console.log('[StoreScreen] Fetching user data...');
          ...
          console.log('[StoreScreen] User data received:', userData);
        } catch (error) {
          console.error("[StoreScreen] User data fetch error:", error);
      ...
      console.error("[StoreScreen] After render error:", error);
    }
  },
...
window.ImpressionHandler = {
  ...
  async syncWithServer() {
    ...
    try {
      const response = await fetch(`${API_URL}/api/sync-impressions`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("accessToken")}`
