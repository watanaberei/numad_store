...
const StoreScreen = {
  render: async () => {
    try {
      ...
      if (store?.location?.neighborhood?.geolocation) {
        storeLocation = [
          store.location.neighborhood.geolocation.lon,
          store.location.neighborhood.geolocation.lat
        ];
      } else {
        console.error("Store location data missing or invalid");
      }
      ...
      try {
        ...
        if (userLocation && storeLocation) {
          ...
          console.log("Store distance:", storeDistance);
        } else {
          console.error("Unable to calculate distance - missing coordinates");
        }
      } catch (error) {
        console.error("Error calculating store distance:", error);
      }
      ////////////// DISTANCE //////////////
