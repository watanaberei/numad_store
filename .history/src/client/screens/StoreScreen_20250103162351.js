...
import * as address from "../components/map/geo/address.js";
...
console.log("debug log: store-init01 - Current store location:", storeLocationData);
...
console.log("debug log: store-init23 - Current store:", currentStore);

const storeAddress = currentStore.storeAddress;
console.log("storeAddress", storeAddress)
const storeRegion = currentStore.storeRegion;
...
console.log("debug log: store-init03 - Store location data:", {
  storeAddress,
  storeRegion,
  storeCity,
  designator,
...
console.log("debug log: store-init03 - Attempting Yelp match with:", {
  name: currentStore?.headline?.text,
  location: matchStoreLocation
});
...
const yelpData = yelpMatch?.basic || null;
    ...
    let storeAreaData = {
      area: currentStore?.media?.area?.description || [],
      images: currentStore?.media?.area?.images || [],
      links: currentStore?.media?.area?.links || []
    }
    ...
    const businessHoursData = {
      ...
      hours: [{
        is_open_now: yelpData?.hours?.[0]?.is_open_now || false,
        open: yelpData?.hours?.[0]?.open || data.store?.item?.[0]?.business?.[0]?.hours || [
          // Default hours if no Yelp or local data available
          {
            day: new Date().getDay(),
            start: "0900",
            end: "1700",
      ...
      name: data.store?.item?.[0]?.name || yelpData?.name || "Business Hours",
      isOpenNow: yelpData?.hours?.[0]?.is_open_now || false,
      currentStatus: yelpData?.hours?.[0]?.is_open_now ? 'Open' : 'Closed'
    };

    console.log("debug log: business-hours01 - Hours data structure:", businessHoursData);

    const storeBusinessData = {
      header: "Business",
      businessHours: businessHoursData.schedule,
      footer: {
        contributionsCount: data.store?.item?.[0]?.location?.stats?.contributions,
        modifiedDate: data.store?.item?.[0]?.location?.modified?.date,
        modifiedTime: data.store?.item?.[0]?.location?.modified?.time,
        commentsCount: data.store?.item?.[0]?.location?.stats?.comments,
        reviewsCount: yelpData?.reviewCount || data.store?.item?.[0]?.location?.stats?.reviews,
        likesCount: data.store?.item?.[0]?.location?.stats?.likes,
        dislikesCount: data.store?.item?.[0]?.location?.stats?.dislikes
      }
    };

    console.log("debug log: store-init05 - Business data for timeline:", {
      rawYelpHours: yelpData?.hours,
      processedSchedule: businessHoursData.schedule,
      fullBusinessData: storeBusinessData
    });
