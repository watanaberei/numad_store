...
const StoreScreen = {
  render: async () => {
    ...
    // console.log("debug log: store-init01 - Initial store data:", store);
      ...
      // Use StoreData directly since it's now a singleton
      const data = await StoreData.getStoreBySlug(request.slug);
      console.log("[StoreScreen.render] Store data:", store);
  ...
  after_render: async () => {
    ...
    if (!chartsContainer) {
      console.log("chartsContainer element not found");
      // Handle missing element case
    }

    const myBtn = document.getElementById('myBtn');
    if (!myBtn) {
      console.log("myBtn element not found");
      // Handle missing element case
    }
