...
console.log("debug log: store-init03 - Current store location:", {
  ...
  title: currentStore?.overviewTitle
    ...
    // YELP MATCH
    const storeCity = currentStore?.location.city; // Changed from location to Location
    const storeState = currentStore?.location.state;
    const storeName = currentStore?.overviewTitle;
    const searchRequest = {
      term:storeName,
      location: storeCity
    };
...
const yelpMatch = await matchBusiness(
  currentStore?.overviewTitle,  // store name
  {
    city: currentStore?.location?.city,    // explicitly pass city
    state: currentStore?.location?.state   // explicitly pass state
  }
);
