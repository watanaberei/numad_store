...
import { resolveApiPath, resolveSocketPath } from '../utils/paths.js';
...
const socket = io(resolveSocketPath());
...
const StoreScreen = {
  render: async () => {
    try {
      ...
      const storeDetails = await fetch(resolveApiPath(`/api/stores/${request.slug}`))
        .then(res => res.json());
      console.log("[StoreScreen] Store data:", storeDetails);
      ...
      // console.log("%06: store", storeDetails);
