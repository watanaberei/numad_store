    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const retryResponse = await fetch(`http://localhost:6000/api/stores/sync/${request.slug}`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
        ...
        // After response
        console.log(`[1StoreScreen] Server response:`, responseData);
      } catch (syncError) {
        ...
        if (syncError.message.includes("Failed to fetch")) {
         ...
         // After response
        console.log(`[2StoreScreen] Server response:`, responseData);
        // Continue rendering even if sync fails
      }
       // After response
       console.log(`[3StoreScreen] Server response:`, responseData);
