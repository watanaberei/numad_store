...
const StoreScreen = {
  ...
  after_render: async () => {
    ...
    if (accessToken) {
      try {
        const userResponse = await fetch(`http://localhost:4000/api/user/impressions/${storeSlug}`, {
          headers: {
            Authorization: `Bearer ${accessToken}`
          }
        });
...
window.storeActions = {
    ...
    fetch("http://localhost:4000/api/store/checkin", {
      ...
      .catch((error) => {
        ...
        if (isCheckingOut) {
          ...
          const impression =
            document.querySelectorAll("#impressions");
  ...
  toggleImpression: async function(storeId, type, section = 'overview') {
    ...
    try {
      ...
      if (result.success) {
        // Update the counts
        const likeCount = document.querySelector(`#userImpressions${type} .count`);
        const dislikeCount = document.querySelector(`#userImpressions${otherType} .count`);
