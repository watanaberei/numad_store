...
import { parseRequestUrl, getCurrentStoreSlug } from "../utils/utils.js";
...
const StoreScreen = {
  render: async () => {
    try {
      ...
      // Post store slug to server
      try {
        const response = await fetch(`${API_URL}/api/stores/current`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          throw new Error(`Failed to post store slug: ${response.statusText}`);
        }

        const result = await response.json();
        // console.log('[StoreScreen] Posted store slug:', result);

      } catch (error) {
        // console.error('[StoreScreen] Failed to post store slug:', error);
        console.error('[StoreScreen] Failed to post store slug:');
      }
      ...
      if (accessToken) {
        try {
          const response = await fetch(`${API_URL}/api/init-store/${request.slug}`, {
            method: 'POST',
            headers: {
          ...
          console.error('[StoreScreen] Failed to record visit:');
        }
      }
      ...
      // console.error("[StoreScreen] Error:");
  ...
  after_render: async () => {
    ...
    if (accessToken) {
      let czc = null;
      try {
        const response = await fetch(`${API_URL}/api/stores/${request.slug}`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
            'Content-Type': 'application/json'
    ...
    try {
      ...
      consolse.error("Error fetching user data:", error);
      // consoles.error("Error fetching user data:");
    }
...
window.storeActions = {
  ...
  toggleImpression: async function(storeId, type) {
    ...
    try {
      ...
      const response = await fetch(`${AUTH_URL}/api/impression`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      console.error('Error updating impression:');
...
window.ImpressionHandler = {
  ...
  async syncWithServer() {
    ...
    try {
      ...
      console.error('Error syncing impressions:');
