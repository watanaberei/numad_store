...
const StoreScreen = {
  render: async () => {
    ...
    // CONTENTFUL
    const contentfulData = await contentfulData.getData();
  ...
  <div class="item">
    ...
    const storeDetailsHTML = sidebar.storeDetails.render(store);
    const storeHeroHTML = components.storeHero.render(storeObject);
    const storeOverviewHTML = components.storeOverview.render(
      store.overview[0].header,
      store.overview[0].text,
      store.overview[0].summary,
      store.overview[0].footer
    );
    const storeExperienceHTML = components.storeExperience.render(
      store.experience[0].header,
      store.experience[0].text,
      store.experience[0].footer,
      store.experience[0].area,
      store.experience[0].attribute
    );
    const storeServiceHTML = components.storeService.render(
      store.service[0].header,
      store.service[0].text,
      store.service[0].category,
      store.service[0].footer
    );
    const storeBusinessHTML = components.storeBusiness.render(store);
    ...
    return `
      ...
      after_render: async () => {
        ...
        try {
          ...
          if (storeService && store.service?.[0]) {
          ...
          if (storeDetails) {
            storeDetails.innerHTML = sidebar.storeDetails.render(store);
          }
          ...
          if (document.getElementById("map-container")) {
            initMap({
              ...
              center: [store.location.geolocation.lon, store.location.geolocation.lat],
              zoom: 13,
