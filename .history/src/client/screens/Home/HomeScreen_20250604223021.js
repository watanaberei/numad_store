...
// src/screens/HomeScreen.js - FIXED VERSION TO PREVENT CHROME CRASHES
...
// FIXED: Global state to prevent multiple initializations
let homeScreenInitialized = false;
let activeRequests = new Set();
let requestCache = new Map();
const MAX_CONCURRENT_REQUESTS = 3;
const CACHE_TTL = 300000; // 5 minutes

// FIXED: Request controller for canceling requests
...
const HomeScreen = {
      ...
      <container class="col03 grid03 primary">
        <div class="col03 container">
          <div class="col03 title">
            <span class="text03">primary</span>
          </div>
          <div class="col03 grid03 container">
            <!-- Loading indicator -->
            <div id="loading-indicator" class="loading-container" style="display: block;">
              <div class="loading-spinner"></div>
              <span class="text03">Loading content...</span>
            </div>
            
            <!-- Error container -->
            <div id="error-container" class="error-container" style="display: none;">
              <div class="error-message">
                <span class="text03">Error loading content. Please try refreshing the page.</span>
                <button id="retry-button" class="btn-retry">Retry</button>
              </div>
            </div>
            
            <div id="postStores" class="gridCollection col03 postStores list" style="display: none;"> 
              <div class="col03 heading">
                <span class="header01">Stores</span>
              </div>
              <div id="postStoresContent" class="grid03 col03">
                <!-- Store content will be loaded here -->
              </div>
            </div>
            
            <div class="list col03 grid03" id="listing-blog" style="display: none;">
              <div class="col03 heading">
            ...
            <div class="list" id="grid03 col03 listing-article" style="display: none;">
              <div class="col03 heading">
            ...
            <div class="list" id="grid03 col03 listing-review" style="display: none;">
              <div class="col03 heading">
            ...
            <div class="list" id="grid03 col03 listing-store" style="display: none;">
              <div class="col03 heading">
                <span class="header01">Nearby Stores</span>
              </div>
              <div id="postListing" class="grid03 col03 postListing card-listing">
                <!-- Store listing content will be loaded here -->
              </div>
            </div>
          </div>
        </div>
      ...
      <container class="col02 vh100 panel persistent secondary">
        <div class="col02 content">
          <div class="col02 title">
            <span class="text03">secondary</span>
          </div>
          <div class="col02 row01 container">
            <div class="col02 item">
              <div id="map-discover" class="col02 s map map-discover">
  ...
  // FIXED: Complete rewrite to prevent infinite requests and Chrome crashes
  after_render: async () => {
...
// FIXED: Safe map initialization with timeout
...
// FIXED: Safe store data loading with retry and caching
async function loadStoreData(maxRetries = 2) {
...
// FIXED: Safe blog data loading with retry and caching
async function loadBlogData(maxRetries = 2) {
  const cacheKey = 'blog-data';
  
  // Check cache first
  const cached = requestCache.get(cacheKey);
  if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
    console.log('[HomeScreen] Using cached blog data');
    return cached.data;
  }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const requestId = `blog-data-${Date.now()}`;
      activeRequests.add(requestId);
      
      try {
        const blogDataPromise = loadBlogFeatures();
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Blog data timeout')), 15000);
        });
        
        const blogData = await Promise.race([blogDataPromise, timeoutPromise]);
      ...
      if (attempt === maxRetries) {
        console.warn('[HomeScreen] All blog data load attempts failed, returning empty array');
        return [];
      }
...
async function loadBlogFeatures() {
  try {
    ...
    if (response.ok) {
      ...
      console.log('[HomeScreen] Blog API response received');
    ...
    if (error.name === 'AbortError') {
      console.error('[HomeScreen] Blog request was aborted due to timeout');
    } else {
...
// FIXED: Safe map components setup
async function setupMapComponents(map, allFeatures) {
  try {
    console.log('[HomeScreen] Setting up map components safely...');
    
    // FIXED: Initialize geocoder with error handling
    try {
      ...
      console.warn('[HomeScreen] Geocoder initialization failed, continuing without it:', error);
    }
    
    // FIXED: Add markers with error handling
    const markers = await addMarkersToMap(map, allFeatures);
    
    // FIXED: Set up map event handlers with error handling
    setupMapEventHandlers(map, allFeatures, markers);
    
    // FIXED: Set up search functionality with error handling
    setupSearchFunctionality(map, markers);
    
    // FIXED: Initialize user location with error handling
    await initializeUserLocation(map, allFeatures);
    
    console.log('[HomeScreen] Map components setup completed successfully');
    
  } catch (error) {
    console.error('[HomeScreen] Error setting up map components:', error);
    // Continue execution even if some components fail
  }
}

// FIXED: Safe marker addition with error handling
async function addMarkersToMap(map, features) {
  ...
  console.log(`[HomeScreen] Adding ${features.length} markers to map safely`);
  
  // FIXED: Process markers in batches to prevent overwhelming the browser
  const batchSize = 50;
    ...
    batch.forEach((feature, index) => {
      try {
        ...
        if (!feature || !feature.geometry || !feature.geometry.coordinates) {
          console.warn(`[HomeScreen] Invalid feature at index ${i + index}:`, feature);
          return;
        }
        
        const coordinates = feature.geometry.coordinates;
        if (!Array.isArray(coordinates) || coordinates.length !== 2) {
          console.warn(`[HomeScreen] Invalid coordinates for feature at index ${i + index}:`, coordinates);
          return;
        }
        
        const [lng, lat] = coordinates;
        if (typeof lng !== 'number' || typeof lat !== 'number' || isNaN(lng) || isNaN(lat)) {
          console.warn(`[HomeScreen] Invalid coordinate values for feature at index ${i + index}:`, { lng, lat });
          return;
        }
        ...
        console.error(`[HomeScreen] Error creating marker for feature at index ${i + index}:`, error);
      }
    });
    
    // FIXED: Allow UI to breathe between batches
...
// FIXED: Safe map event handlers
function setupMapEventHandlers(map, allFeatures, markers) {
  try {
    // FIXED: Debounced moveend event to prevent excessive calls
    let moveendTimeout;
    ...
    // FIXED: Safe click event listener
...
// FIXED: Safe search functionality
...
// FIXED: Safe user location initialization
async function initializeUserLocation(map, features) {
  try {
    console.log('[HomeScreen] Initializing user location safely...');
    
    if (!navigator.geolocation) {
      console.log('[HomeScreen] Geolocation not available, rendering features without location');
...
// FIXED: Safe feature rendering with error handling
function renderFeatures(features, map) {
  console.log('[HomeScreen] Rendering features safely:', features?.length || 0);

  const elements = {
    postListing: document.getElementById("postListing"),
    postStores: document.getElementById("postStoresContent"),
  ...
  Object.values(elements).forEach(el => {
    if (el) {
      try {
        ...
        console.error('[HomeScreen] Error clearing element:', error);
      }
    }
  });
    ...
    Object.entries(elements).forEach(([key, el]) => {
      if (el) {
        try {
          ...
          console.error('[HomeScreen] Error setting empty state:', error);
        }
      }
    });
    return;
  ...
  // FIXED: Process features in batches to prevent UI blocking
  const batchSize = 10;
  let index = 0;
  ...
  const processBatch = () => {
    ...
    batch.forEach((feature, batchIndex) => {
      try {
        ...
        console.log(`[HomeScreen] Processing feature ${globalIndex}:`, feature?.properties?.variant);
        ...
        console.error(`[HomeScreen] Error processing feature ${index + batchIndex}:`, error);
      }
    });
...
function getUserCoordinates() {
  try {
    // Return default coordinates or implement geolocation
    return [0, 0];
  } catch (error) {
    console.error('[HomeScreen] Error getting user coordinates:', error);
    return [0, 0];
  }
}
  ...
  try {
    ...
    const zoomOut = () => {
      if (zoomAttempts >= maxZoomAttempts) {
        console.log('[HomeScreen] Max zoom attempts reached');
        return;
      }
...
// FIXED: Clean up function to prevent memory leaks
