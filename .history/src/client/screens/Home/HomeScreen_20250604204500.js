...
const HomeScreen = {
  ...
  after_render: async () => {
    ...
    try {
      // Step 1: Initialize map first (this should be fast)
      ...
      // Step 2: Load store data with timeout and retry
      console.log('[HomeScreen] Step 2: Loading store data...');
      const storeFeatures = await loadStoreDataWithRetry();
      
      // Step 3: Load blog data with timeout and retry
      console.log('[HomeScreen] Step 3: Loading blog data...');
      const blogFeatures = await loadBlogDataWithRetry();
      
      // Step 4: Combine and process features
      console.log('[HomeScreen] Step 4: Processing features...');
      const allFeatures = [...storeFeatures, ...blogFeatures];
      console.log(`[HomeScreen] Total features loaded: ${allFeatures.length}`);
      
      // Step 5: Initialize geocoder (optional, don't fail if this breaks)
      console.log('[HomeScreen] Step 5: Initializing geocoder...');
      try {
        await initializeGeocoder(map);
      } catch (error) {
        console.warn('[HomeScreen] Geocoder initialization failed, continuing without it:', error);
      }
      
      // Step 6: Add markers to map
      console.log('[HomeScreen] Step 6: Adding markers to map...');
      const markers = await addMarkersToMap(map, allFeatures);
      
      // Step 7: Set up map event handlers
      console.log('[HomeScreen] Step 7: Setting up map events...');
      setupMapEventHandlers(map, allFeatures, markers);
      
      // Step 8: Set up search functionality
      console.log('[HomeScreen] Step 8: Setting up search...');
      setupSearchFunctionality(map, markers);
      
      // Step 9: Initialize user location and render initial features
      console.log('[HomeScreen] Step 9: Setting up user location...');
      await initializeUserLocation(map, allFeatures);
      
      // Step 10: Hide loading and show content
      hideLoading();
      showContent();
      ...
      if (retryButton) {
        retryButton.onclick = () => {
          ...
          window.location.reload(); // Simple retry by reloading page
        };
      }
    }
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      if (map.loaded()) {
        ...
        map.on('error', (e) => reject(new Error(`Map load error: ${e.error?.message || 'Unknown error'}`)));
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const storeData = await Promise.race([
        geojsonStore(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Store data timeout')), 15000))
      ]);
      ...
      if (attempt === maxRetries) {
        console.warn('[HomeScreen] All store data load attempts failed, continuing with empty array');
        return [];
      }
