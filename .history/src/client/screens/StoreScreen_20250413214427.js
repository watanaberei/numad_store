...
const StoreScreen = {
  ...
  const checkInResponse = await fetch("http://localhost:4000/api/store/checkin/status", {
    headers: {
      Authorization: `Bearer ${accessToken}`
    }
  });
  ...
  if (checkInResponse.ok) {
    const checkInStatus = await checkInResponse.json();
    
    // Check if user is already checked-in to this store
    if (checkInStatus.success && checkInStatus.checkedInStore === storeSlug) {
