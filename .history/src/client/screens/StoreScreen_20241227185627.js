...
const StoreScreen = {
  render: async () => {
    ...
    console.log("debug log: store-init01 - Parsing request URL:", request);
    ...
    console.log("debug log: store-init03 - Current store:", currentStore);

    // YELP DATA
    const yelpData = yelpApi.yelpApi;
    console.log("debug log: store-init04 - Yelp data:", yelpData);

    // LOCAL DATA
    const localData = data.store?.item?.[0];
    console.log("debug log: store-init05 - Local data:", localData);
    ...
    const storeLocationData = {
      header: {
        title:
          contentfulData?.location?.city +
          ", " +
          contentfulData?.location?.area,
        neustar: contentfulData?.neustar || 0
      },
    ...
    let storeObject = {
      ...
      attributesFacility:
        ...
        contentfulData.summary.facility.length
          ? contentfulData.summary.facility
          : [],
      attributesExperience:
        contentfulData.summary.experience &&
        contentfulData.summary.experience.length
          ? contentfulData.summary.experience
          : [],
      attributesOverview:
  ...
  <div class="item">
    ...
    let storeServices = {
      ...
      attributeService:
        contentfulData?.summary?.service &&
        contentfulData?.summary?.service?.length
          ? contentfulData?.summary?.service
          : [],
      snippetService: contentfulData?.snippet?.service || [],
      mediaTopThree:
