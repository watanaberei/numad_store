  ...
  render: async () => {
    try {
      ...
      if (!store) {
        ...
        try {
          const mongoResponse = await fetch(`http://localhost:8080/api/stores/${request.slug}`);
          ...
          if (mongoResponse.ok && mongoData.success && mongoData.store) {
            ...
            console.log("[StoreScreen] Store not found in MongoDB either");
            // Create a basic placeholder store
            store = {
              slug: request.slug,
              hero: {
                storeName: request.slug.split('_').pop().replace(/-/g, ' '),
                storeType: "Store",
                rating: "0",
                price: "$$",
                distance: "Unknown",
              ...
              overview: [{
                header: "Overview",
                text: { title: "Store Information", content: "Store information is currently unavailable." },
                footer: {}
              }],
              experience: {
                header: "Experience",
      ...
      if (store) {
        try {
          ...
          const response = await fetch(`http://localhost:8080/api/stores/sync/${request.slug}`, {
            method: 'POST',
            headers: {
          ...
          if (response.ok) {
            ...
            if (responseData.success) {
              console.log(`[StoreScreen] Store data successfully saved to MongoDB: ${responseData.store?.title || request.slug}`);
            } else {
              console.error("[StoreScreen] Failed to save store data to MongoDB:", responseData.message);
            }
          } else {
            console.error("[StoreScreen] HTTP error saving to MongoDB:", response.status);
          }
        } catch (syncError) {
...
window.storeActions = {
  ...
  toggleImpression: async function (storeId, type) {
    ...
    try {
      const response = await fetch("http://localhost:8080/api/impression", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
