    ...
    try {
      ...
      if (!store) {
        ...
        try {
          ...
          const mongoResponse = await fetch(`http://localhost:6000s /stores/${request.slug}`);
      ...
      try {
        ...
        const response = await fetch(`http://localhost:6000/stores/${request.slug}`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          ...
          const retryResponse = await fetch(`http://localhost:6000/stores/${request.slug}`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
