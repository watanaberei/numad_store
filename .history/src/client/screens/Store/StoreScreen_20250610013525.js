...
const StoreScreen = {
  ...
  after_render: async () => {
    console.log("[StoreScreen line 126] Starting after_render...");
    ...
    // Initialize interaction handlers and socket.io
    ...
    try {
      ...
      console.log(`[StoreScreen line 143] Fetching store data for slug: ${storeSlug}`);
      ...
      // 3. Fetch store data from MongoDB API
      console.log(`[StoreScreen line 151] Making API call to: http://localhost:4500/api/stores/${storeSlug}`);
      ...
      console.log(`[StoreScreen line 163] API response status: ${response.status}`);
      ...
      console.log('[StoreScreen line 170] Store data received:', data);
      ...
      // 4. Store data is now available in the exact same structure
      console.log("[StoreScreen line 179] Store sections available:", {
        hero: !!store.hero,
        overview: !!store.overview,
        service: !!store.service,
        experience: !!store.experience,
        location: !!store.location,
        business: !!store.business,
        yelpData: !!store.yelpData
      });
      
      // 5. Extract data sections (same structure as before)
      const {
        hero = {},
        overview = [],
        service = {},
      ...
      // 6. Generate HTML using existing components
      const storeHTML = `
        <div id="section" class="store hero col05">
          <div class="col05 array content" id="store-hero">
            ${components.storeHero.render(hero)}
      ...
      if (contentEl) {
        contentEl.innerHTML = storeHTML;
        contentEl.style.display = 'block';
      }
      
      console.log('[StoreScreen line 254] Store rendered successfully');
      
      // 8. Initialize components with store data
      await initializeStoreComponents(store);
      
      // 9. Record visit if logged in
      if (localStorage.getItem('accessToken')) {
        try {
          ...
          console.log('[StoreScreen line 270] Store visit recorded');
        } catch (error) {
          console.warn('[StoreScreen line 272] Could not record visit:', error);
        }
      }
      
      // 10. Check user interaction status
      await checkUserInteractionStatus(storeSlug);
      
    } catch (error) {
      console.error('[StoreScreen line 280] Error loading store:', error);
