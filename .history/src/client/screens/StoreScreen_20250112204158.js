...
const StoreScreen = {
  render: async () => {
    ...
    try {
      ...
      const combinedStore = await storeData.getStoreBySlug(request.slug);
      ...
      const storeHeroData = combinedStore.yelpData?.data || {};
      const storeOverviewData = combinedStore?.overview;
      const storeServiceData = combinedStore?.service;
      const storeExperienceData = combinedStore?.experience;
      const storeLocationData = combinedStore?.location;
      const storeBusinessData = combinedStore?.business;
      const storeDetailData = combinedStore.yelpData?.data || {};
