    ...
    features.forEach(function (marker) {
      ...
      el.addEventListener('click', async () => {
        try {
          ...
          const response = await fetch('http://localhost:4000/api/store-first-click', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              storeId: marker.properties.slug,
              storeData: {
                ...
                category: {
                  categoryType: marker.properties.categoryType
                }
              }
            })
          });
          ...
          if (!response.ok) {
            console.error('Error logging store click:', await response.text());
          }
        } catch (error) {
          console.error('Error sending store click:', error);
        }
      });
