...
const StoreScreen = {
  render: async () => {
    try {
      ...
      if (!store) {
        console.log("[StoreScreen] Store not found in data.js, checking MongoDB...");
  ...
  after_render: async () => {
    ...
    try {
      ...
      if (userResponse.ok) {
        ...
        const checkedInStore = userData.checkedInStore;
        if (checkedInStore && checkedInStore === storeSlug) {
    ...
    const userControl = userControl.render(store, {
...
window.storeActions = {
  ...
  toggleCheckInStore: function(storeName, storeId) {
    const checkinButton = document.getElementById('storeControls-checkin');
    const userImpression = document.getElementById('userImpression');
    ...
    try {
      ...
      // Send check-in/check-out status to server using regular fetch
      fetch('http://localhost:4000/api/store/checkin', {
        ...
        if (!response.ok) {
          console.warn('[StoreScreen] Check-in request failed, but UI was updated');
        }
        return response.json();
      }).then(data => {
        console.log('[StoreScreen] Check-in response:', data);
      }).catch(error => {
        console.error('[StoreScreen] Error during check-in:', error);
      });
      
      // Show confirmation to user
      alert(`${isCheckingOut ? 'Checked out from' : 'Checked in to'} ${storeName}`);
      
    } catch (error) {
      console.error('[StoreScreen] Error in toggleCheckInStore:', error);
    }
  },
...
window.storeActions = {
  ...
  // Updated toggleCheckInStore function to handle server communication
  toggleCheckInStore: async function(storeName, storeId) {
    const checkinButton = document.getElementById('storeControls-checkin');
    const userImpression = document.getElementById('userImpression');
    ...
    try {
      ...
      // Send check-in/check-out status to server
      const response = await fetch('http://localhost:4000/api/store/checkin', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken}`
      ...
      if (!response.ok) {
        ...
        console.log("[StoreScreen.js.toggleCheckInStore] Server request failed, updating UI optimistically");
        updateCheckInUI();
        return;
      }
      ...
      // Still update UI optimistically on error
      updateCheckInUI();
    }
    ...
    function updateCheckInUI() {
      ...
      if (isCheckingOut) {
        console.log("[StoreScreen.js.toggleCheckInStore] Checking out");
        checkinButton.classList.remove('checked-in');
        checkinButton.querySelector('span').textContent = 'Check-in';
        if (userImpression) userImpression.classList.add('disabled');
        alert(`Checked out from ${storeName}`);
        ...
        console.log("[StoreScreen.js.toggleCheckInStore] Checking in");
        checkinButton.classList.add('checked-in');
        checkinButton.querySelector('span').textContent = 'Checked-in';
        if (userImpression) userImpression.classList.remove('disabled');
        alert(`Checked in to ${storeName}`);
  ...
  toggleImpression: async function(storeId, type) {
    ...
    // Check if the user is checked in first
    const checkinButton = document.getElementById('storeControls-checkin');
