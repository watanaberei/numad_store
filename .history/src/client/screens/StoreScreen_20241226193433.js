...
const StoreScreen = {
  async init() {
    try {
      ...
      console.log("debug log: Store data received", storeData);
      
      // Extract data structures
      this.storeOverviewData = storeData?.store?.item?.[0]?.overview?.[0];
      this.storeServiceData = storeData?.store?.item?.[0]?.service?.[0];
      this.storeExperienceData = storeData?.store?.item?.[0]?.experience?.[0];
      this.storeHeroData = storeData?.store?.item?.[0]?.hero?.[0];
      this.storeLocationData = storeData?.store?.item?.[0]?.location;
      this.storeBusinessData = yelpApi.yelpApi;

      console.log("debug log: Data structures extracted");
    } catch (error) {
  ...
  render: async () => {
    try {
      ...
      console.log('debug log: Store request:', request);

      try {
        // Get store data from Contentful using slug
        const stores = await getStoresNeumadsReview();
        console.log('All stores:', stores);
        
        const store = stores.find(s => s.slug === request.slug);
        ...
        console.error('Error in StoreScreen:', error);
        return `<div>Error loading store</div>`;
      }
    } catch (error) {
      console.error('Error in render:', error);
      return `<div>Error initializing store</div>`;
    }
  },
  ...
  after_render: async () => {
    try {
      ...
      // Other UI Components
      StoreScreen.renderUIComponents(store, storeData);
