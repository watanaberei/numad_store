...
const StoreScreen = {
  render: async () => {
    ...
    try {
      ...
      const storeData = data.store.item.find(store => 
        store.name === "Smoking Tiger Bread Factory" // TODO: Make this dynamic based on request
      );
      console.log("[StoreScreen.render] Found store data:", storeData);

      if (!storeData) {
        console.error("[StoreScreen.render] Store data not found");
        return `<div>Store not found</div>`;
      }

      // 3. Extract all the required data sections
      const storeHeroData = storeData?.hero?.[0];
      const storeOverviewData = storeData?.overview?.[0];
      const storeServiceData = storeData?.service?.[0];
      const storeExperienceData = storeData?.experience?.[0];
      const storeLocationData = storeData?.location;
      const storeBusinessData = storeData?.business?.[0];
      const storeDetailData = storeData?.detail?.[0];
