...
const PostScreen = {
  ...
  after_render: async () => {
    ...
    async function savePost(status, isAutosave = true) {
      try {
        ...
        console.log('formData.get("snippet")', formData.get("snippet"));
        console.log('formData.get("category")', formData.get("category"));
        console.log('formData.get("tags")', formData.get("tags"));
        console.log('formData.get("template")', formData.get("template"));
        console.log('formData.get("coverImage")', formData.get("coverImage"));
        console.log('formData.get("status")', formData.get("status"));
        ...
        const completeBlogData = {
          ...
          author: {
            ...
            slugAuthor: formData.get("authorSlug"),
            social: formData.get("authorSocial"),
            email: formData.get("authorEmail"),
            id: formData.get("authorId")
          }
        }
        console.log('completeBlogData', completeBlogData);
        // Get access token
        const accessToken = localStorage.getItem('accessToken');
        if (!accessToken) {
          if (!isAutosave) {
        ...
        // Convert formData (which is a FormData object) to a plain object for JSON
        // If you want to support file uploads, you must use FormData and not JSON
        // But your backend expects JSON, so we convert it here
        const plainFormData = {};
