      ...
      async function fetchPublicProfileData(username) {
        try {
          ...
          if (!response.ok && response.status === 404) {
            console.log('[ProfileScreen] Trying with original case:', username);
          ...
          if (!profile.blogPosts && profile.stats) {
            ...
            try {
              ...
              if (blogsResponse.ok) {
                ...
                console.log('[ProfileScreen] Fetched blog posts separately:', profile.blogPosts.length);
              }
            } catch (blogError) {
              console.error('[ProfileScreen] Error fetching blogs separately:', blogError);
              profile.blogPosts = [];
            }
          }
