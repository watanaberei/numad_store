...
import { geojsonStore } from "../../components/cards/store/cardStoreGeojsonListing.js";
  ...
  render: async () => {
        ...
        <div id="profile-container" class="col05 profile-container" style="display: none;">
          ...
          <section class="profile-carousels-section col05 row01">
            ...
            <div id="recently-visited-carousel col05 row01" class="content-carousel" style="display: none;">
              ...
              <div id="recently-visited-content" class="carousel-wrapper col05 row01">
                <div class="loading-placeholder">Loading recently visited places...</div>
              </div>
            </div>

            <!-- Blog Posts Carousel -->
            <!-- <div id="blog-posts-carousel" class="content-carousel col05 row01" style="display: none;"> -->
            <div id="blog-posts-carousel" class="content-carousel col05 row01" style="display: block;">
              <div class="carousel-header col05 row01">
                <h3 class="carousel-title">Recent Posts</h3>
                <button class="carousel-view-all" data-target="posts">View All</button>
              </div>
              <div id="blog-posts-content" class="carousel-wrapper col05 row01">
    ...
    after_render: async () => {
      ...
      async function fetchPublicProfileData(username) {
        try {
          console.log('fetchPublicProfileData [ProfileScreen] Fetching public profile for:', username);
          
          // Try with the username as provided (API will do case-insensitive search)
          ...
          // If lowercase fails, try with original case
          if (!response.ok && response.status === 404) {
            console.log('fetchPublicProfileData [ProfileScreen] Trying with original case:', username);
            response = await fetch(`${SERVER_URL}/api/user/${username}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json'
              }
          ...
          // The API returns the profile in data.profile
          const profile = data.profile || data.user || data;
          
          // Ensure blog data is included
          if (!profile.blogPosts && profile.stats) {
            // Fetch blog posts separately if not included
            try {
              const blogsResponse = await fetch(`${SERVER_URL}/api/user/${username.toLowerCase()}/blogs?status=published&limit=20`);
              if (blogsResponse.ok) {
                const blogsData = await blogsResponse.json();
                profile.blogPosts = blogsData.blogs || [];
                console.log('[fetchPublicProfileData ProfileScreen] Fetched blog posts separately:', profile.blogPosts.length);
              }
            } catch (blogError) {
              console.error('fetchPublicProfileData [ProfileScreen] Error fetching blogs separately:', blogError);
              profile.blogPosts = [];
            }
          }
          ...
          console.error('fetchPublicProfileData [ProfileScreen] Error fetching public profile:', error);
          throw error;
        }
      }
     ...
     // Load blog posts carousel (DEBUG VERSION)
      ...
      async function loadPublicProfileContent(username) {
        try {
          console.log('loadPublicProfileContent [ProfileScreen] Loading public profile content for:', username);
      ...
      async function loadPostsContent(isOwn, username = null) {
        try {
          ...
          if (!targetUsername) {
            postsContent.innerHTML = '<div class="error-state"><p>Username not available</p></div>';
            return;
          }
          console.log('[ProfileScreen] Loading posts content for:', targetUsername);
          ...
          if (response.ok) {
            ...
            console.error('[ProfileScreen] Failed to load posts:', response.status);
