...
const StoreScreen = {
  render: async () => {
    ...
    console.log("debug log: store01 - Parsing request URL:", request);

    // Create a mutable copy of the store data
    const storeData = {
      ...contentfulData,
      location: {
        ...contentfulData?.location,
        coordinates: {
          latitude: contentfulData?.location?.geolocation?.lat || 0,
          longitude: contentfulData?.location?.geolocation?.lon || 0
        }
      }
    };
    ...
    const storeLocationData = {
      header: {
        title: `${storeData?.location?.city || ''}, ${storeData?.location?.area || ''}`,
        neustar: storeData?.neustar || 0
      },
      attribute: storeData?.location?.attribute || {},
      coordinates: {
        lat: storeData?.location?.coordinates?.latitude,
        lon: storeData?.location?.coordinates?.longitude
      },
      footer: {
        contributionsCount: storeData?.stats?.contributions || 0,
        modifiedDate: storeData?.modified?.date || 'N/A',
        modifiedTime: storeData?.modified?.time || 0,
        commentsCount: storeData?.stats?.comments || 0,
        reviewsCount: storeData?.stats?.reviews || 0,
        likesCount: storeData?.stats?.likes || 0,
        dislikesCount: storeData?.stats?.dislikes || 0
      }
    };

    console.log("debug log: store02 - Store location data:", storeLocationData);
    ...
    if (storeLocationHTML && storeLocationData) {
      try {
        ...
        // Pass the coordinates to afterRender
    ...
    contentfulData = validStores.find((contentfulData) => contentfulData.slug === request.slug);
    console.log("slug", contentfulData.slug);
    console.log("store", contentfulData);
    ...
    const storeHeroData = {
        ...
        storeName: contentfulData?.overviewTitle,
        storeCity: contentfulData?.location?.city,
        distanceMiles: storeDistance,
        neustar: contentfulData?.neustar || 0,


        galleryImages: contentfulData?.galleryImages,


        rating: contentfulData?.rating,
        priceRange: contentfulData?.costEstimate,
        storeType: contentfulData?.storeType,
        distance: contentfulData?.distance,
        distanceMiles: storeDistance,
        city: contentfulData?.location?.city,
        state: contentfulData?.location?.state,
        neustar: contentfulData?.neustar || 0,
    ...
    const storeLocationData = {
      header: {
        title: contentfulData?.location?.city + ", " + contentfulData?.location?.area,
        neustar: contentfulData?.neustar || 0,
      },
      address: data.store?.item?.[0]?.location?.attribute,
      coordinates: {
        lat: contentfulData?.location?.geolocation?.lat || 0,
        lon: contentfulData?.location?.geolocation?.lon || 0
      },
      footer: {
        contributionsCount: data.store?.item?.[0]?.location?.stats?.contributions,
    ...
    const coordinateStore = Geolocate.coordinateStore(contentfulData);
    const storeLocation = [
      contentfulData?.location?.geolocation?.lon,
      contentfulData?.location?.geolocation?.lat,
    ];
    ...
    // MEDIA
    const mediaArea =
    contentfulData.media.area &&
      Array.isArray(contentfulData.media.area) &&
      contentfulData.media.area.length
        ? contentfulData.media.area
        : [];
    const mediaGallery =
    contentfulData.media.gallery &&
      Array.isArray(contentfulData.media.gallery) &&
      contentfulData.media.gallery.length
        ? contentfulData.media.gallery
        : [];
    const mediaService =
    contentfulData.media.service &&
      Array.isArray(contentfulData.media.service) &&
      contentfulData.media.service.length
        ? contentfulData.media.service
        : [];
    // CAROUSEL
    ...
    //////////////////////////// SNIPPET ////////////////////////////
    const snippetOverview = contentfulData.snippet.overview || [];
    const snippetFacility = contentfulData.snippet.facility || [];
    const snippetExperience = contentfulData.snippet.experience || [];
    const snippetService = contentfulData.snippet.service || [];
    const snippetLocation = contentfulData.snippet.location || [];
    const snippetHours = contentfulData.snippet.hours || [];

    const popularTimesData = contentfulData.popularTimes || [];

    const thumbnails = contentfulData?.media?.thumbnail;
    const heroModuleHtml = hero.heroModule.render(contentfulData.media.hero);
    const neustar = contentfulData.neustar;
    const headline = contentfulData?.headline?.text;
    const locationRegion = contentfulData?.location?.region;
    // console.log("!#$#%#@$%!$#%$!#", headline, locationRegion);

    const limitedBest02 = contentfulData?.summary?.best?.length
      ? contentfulData.summary.best.slice(0, 3)
      : [];
    // ATTRIBUTES
    ...
    let headlineObject = {
      headlineText: contentfulData.headline.text,
      locationRegion: contentfulData.location.region,
      attributesArrays: attributesArray,
    };

    const storeCoordinates = Geolocate.coordinateStore(contentfulData);
    ...
    let storeObject = {
      ...
      storeName: contentfulData?.headline?.text || "Default Headline", // Provide a default value
      city: contentfulData?.location?.region || "Default Region", // Provide a default value
      attributesArrays: attributesArray,
      storeRegion: contentfulData.region,
      storeURL: contentfulData.url,
      ...
      distanceMiles: storeDistance,
      storeTypes: contentfulData.category.genre,
      storeType: contentfulData.category.categoryType,
      storeCategory: contentfulData.location.type,

      // ATTRIBUTES
      attributesBest: contentfulData?.summary?.best?.length
        ? contentfulData.summary.best.slice(0, 3)
        : [],
      attributesFacility:
        contentfulData.summary.facility && contentfulData.summary.facility.length
          ? contentfulData.summary.facility
          : [],
      attributesExperience:
        contentfulData.summary.experience && contentfulData.summary.experience.length
          ? contentfulData.summary.experience
          : [],
      attributesOverview:
        contentfulData.summary && contentfulData.summary.text && contentfulData.summary.text.length
          ? contentfulData.summary.text
          : [],
      attributeService:
        contentfulData.summary.service && contentfulData.summary.service.length
          ? contentfulData.summary.service
          : [],
      ...
      mediaArea:
        ...
        contentfulData.media.area.length
          ? contentfulData.media.area
          : [],
      storeArea: contentfulData?.media?.area?.description || [],
      snippetOverview: contentfulData?.snippet?.overview || [],
      snippetFacility: contentfulData?.snippet?.facility || [],
      snippetExperience: contentfulData?.snippet?.experience || [],
      snippetService: contentfulData?.snippet?.service || [],
  ...
  <div class="item">
    ...
    let storeIntro = {
      ...
      recommendValue: "90%",
      snippetOverview: contentfulData.summaryDetails,
      title: contentfulData.summaryText,
      neustar: neustar,
      buttonFloating: "labelButton",
      headlineText: contentfulData?.headline?.text || "Default Headline", // Provide a default value
      locationRegion: contentfulData?.location?.region || "Default Region", // Provide a default value
      attributesArrays: attributesArray,
      storeType: contentfulData.category.categoryType,
      storeCategory: contentfulData.location.type,
    };
    ...
    const nearbyStore = contentfulData.nearbyStore || [];
    ...
    const nearbyStores = contentfulData.nearbyStore || [];
    ...
    let storeFacility = {
      ...
      nearbyStore: contentfulData?.nearbyStore || [],
      nearbyHeadlines: contentfulData?.nearbyStore?.nearbyHeadline || [],
      nearbyHeadline:
        contentfulData?.nearbyStoresCollection?.items?.nearbyHeadline &&
        Array.isArray(contentfulData?.nearbyStoresCollection?.items?.nearbyHeadline) &&
        contentfulData?.nearbyStoresCollection?.items?.nearbyHeadline?.length
          ? contentfulData?.nearbyStoresCollection?.items?.nearbyHeadline
          : [],
      ...
      mediaArea:
        ...
        contentfulData?.media?.area?.length
          ...
          : [],
      snippetFacility: contentfulData.snippet.facility || [],
      popularTimes: contentfulData.popularTimes || [],
    ...
    ///////////////////////////////////////////////////////////////
    let storeDetails = {
      storeGenre: contentfulData?.category?.genre,
      storeType: contentfulData?.category?.categoryType,
      storeContact: contentfulData?.contact,
      storeAddress: contentfulData?.location?.address || [],
      storeLocatedIn: contentfulData?.location?.locatedIn,
      storeRegion: contentfulData?.location?.region || "Default Region", // Provide a default value
      //   currentDistance: MapDistance.calculateDistance(
      //     userLocation,
      //     storeLocation
      //   ),
      ...
      storeDistance: storeDistance,
      storeName: contentfulData?.headline?.text || "Default Headline", // Provide a default value
      storeHours: contentfulData?.hours,
      storeRatings: contentfulData?.ratings[0]?.key || [],
      storeRatingGoogle: contentfulData?.googleRatings,
      storeRatingYelp: contentfulData?.yelpRatings,
      storeHandle: contentfulData?.handles,
      storeReviews: contentfulData?.ratings[0]?.value || [],
      storeBest: attributesArray,
      storeLogo: contentfulData?.media?.logo,
      storeNeustar: contentfulData?.neustar,
    };
    ...
    return `
      ...
      after_render: async () => {
        ...
        //     if (storeHero && contentfulData.hero?.[0]) {
