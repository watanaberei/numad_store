...
const StoreScreen = {
  async init() {
    try {
      console.log("StoreScreen.jsdebug log: Initializing application");
      
      // Get store data using ContentfulData
      const contentful = await contentfulData.getStoreId();
      console.log("contentfulData in StoreScreen.js", contentful);
      
      // Extract data structures from the fetched store data
      const store = contentful?.features?.[0]?.properties;
  ...
  render: async () => {
    try {
      ...
      const { contentful } = await data.initializeData();
      const request = parseRequestUrl();

      console.log('debug log: storeScreen init15 Store request:', request); // Debug log

      console.log("debug log: store-init03 - Store data:", contentful);
  ...
  after_render: async () => {
    try {
      ...
      if (storeDetails && contentful) {
        storeDetails.innerHTML = sidebar.storeDetails.render(contentful);
      }
      ...
      if (storeBusiness && contentful?.hours) {
        storeBusiness.innerHTML = components.storeBusiness.render(contentful);
        components.storeBusiness.afterRender();
      }

      // Other UI Components
      this.renderUIComponents(store, contentful);
  ...
  renderUIComponents: (store, contentful) => {
    // Footer
