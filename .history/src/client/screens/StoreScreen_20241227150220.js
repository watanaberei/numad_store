...
const StoreScreen = {
  render: async () => {
    ...
    const storeLocationData = {
      header: {
        title: data.store?.item?.[0]?.location?.city + ", " + data.store?.item?.[0]?.location?.area,
        neustar: contenful?.neustar || 0
      },
      attribute: data.store?.item?.[0]?.location?.attribute,
      coordinates: {
        lat: data.store?.item?.[0]?.location?.coordinates?.latitude || contenful?.location?.geolocation?.lat || 0,
        lon: data.store?.item?.[0]?.location?.coordinates?.longitude || contenful?.location?.geolocation?.lon || 0
      },
      footer: {
    ...
    console.log("debug log: location01 - Store location data:", storeLocationData);
  ...
  <div class="item">
    ...
    if (storeLocationElement && storeLocationData) {
      try {
        ...
        // Call afterRender to initialize map and other components
        components.storeLocation.afterRender(storeLocationData.coordinates);
      } catch (error) {
