...
const UserScreen = {
  render: async () => {
    ...
    let userData = {};
    let checkedInStore = null;

    try {
      // Get check-in status using the store/checkin/status endpoint
      const response = await fetch(`${API_URL}/api/store/checkin/status`, {
        headers: {
      ...
      if (response.ok) {
        ...
        console.log("Successfully fetched check-in status:", data);
        
        if (data.success && data.checkedInStore) {
          checkedInStore = data.checkedInStore;
        }
      } else {
        console.error('Failed to fetch check-in status, status:', response.status);
      }
    } catch (error) {
                                ...
                                <div id="checkedInStoresContent">
                                  ${checkedInStore ? 
                                    `<div class="store-card">
                                      <h3>Currently checked into: ${checkedInStore}</h3>
                                     </div>` : 
                                    '<p>Not currently checked in to any store</p>'}
                                </div>
                              </div>
                            </div>
    ...
    after_render: async () => {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          console.log('Check-in status from API:', data);
          
          if (data.success && data.checkedInStore) {
            try {
              // Try to get store details by ID (if you have an endpoint for this)
              const storeDetailResponse = await fetch(`${API_URL}/api/store/${data.checkedInStore}`, {
                headers: {
                  'Authorization': `Bearer ${accessToken}`
                }
              }).catch(err => {
                // Silently handle missing endpoint
                console.log('Store detail endpoint might not exist:', err);
                return { ok: false };
              });
              
              if (storeDetailResponse && storeDetailResponse.ok) {
                const storeData = await storeDetailResponse.json();
                // Update the UI with detailed store info
                checkedInStoresContent.innerHTML = `
                  <div class="store-card">
                    <h3>${storeData.name || storeData.title || data.checkedInStore}</h3>
                    <p>${storeData.address || ''}</p>
                    <p>Check-in time: ${new Date().toLocaleString()}</p>
                  </div>
                `;
              } else {
                // If we can't get details, just show the store ID
                checkedInStoresContent.innerHTML = `
                  <div class="store-card">
                    <h3>Currently checked into: ${data.checkedInStore}</h3>
                  </div>
                `;
              }
            } catch (error) {
              console.warn('Error fetching store details:', error);
              checkedInStoresContent.innerHTML = `
                <div class="store-card">
                  <h3>Currently checked into: ${data.checkedInStore}</h3>
                </div>
          ...
          console.error('Failed to fetch check-in status, status:', response.status);
          checkedInStoresContent.innerHTML = '<p>Failed to load check-in status</p>';
        }
        
        // Also try to get user data for visit history and other info
        const userDataResponse = await fetch(`${API_URL}/api/user`, {
          headers: {
            'Authorization': `Bearer ${accessToken}`
          }
        });
        ...
        if (userDataResponse.ok) {
          ...
          // Update saved stores
          if (savedStoresContent && userData.savedStores && userData.savedStores.length > 0) {
            let savedHTML = '';
            for (let storeId of userData.savedStores) {
              savedHTML += `
            ...
            savedStoresContent.innerHTML = savedHTML;
          } else if (savedStoresContent) {
            savedStoresContent.innerHTML = '<p>No saved stores</p>';
          }
          
          // Update visit history
          if (recentlyVisitedContent && userData.visitHistory && userData.visitHistory.length > 0) {
            let visitHTML = '';
            // Show most recent 5 visits
            const recentVisits = userData.visitHistory.slice(-5).reverse();
            for (let visit of recentVisits) {
              visitHTML += `
                <div class="visit-item">
                  <h4>${visit.storeId}</h4>
                  <p>Visited: ${new Date(visit.timestamp).toLocaleString()}</p>
                </div>
              `;
            }
            recentlyVisitedContent.innerHTML = visitHTML;
          } else if (recentlyVisitedContent) {
            recentlyVisitedContent.innerHTML = '<p>No recent visits</p>';
          }
        } else {
