...
const StoreScreen = {
  render: async () => {
    ...
    try {
      ...
      const storeData = new data.StoreData(request.slug);
      ...
      // Map the data to match original structure
      const mappedData = {
        hero: {
          headline: store.headline?.text,
          subtext: store.headline?.subtext,
          media: store.media?.hero,
          gallery: store.media?.gallery,
          rating: store.yelpRatings?.[0]?.key || "0",
          reviewCount: store.yelpRatings?.[0]?.value || "0",
          price: store.price,
          storeType: store.tagsCollection?.items[0]?.storeTags || [],
          distance: store.distance
        },
        overview: {
          attributes: store.attributesOverview || [],
          best: store.tagsCollection?.items[0]?.attributeTags || [],
          environment: store.tagsCollection?.items[0]?.environmentTags || [],
          location: store.locationCollection?.items[0] || {}
        },
        service: {
          attributes: store.storeServices || [],
          menu: store.attributeService || [],
          hours: store.popularTimes || []
        },
        experience: {
          attributes: store.attributes?.experiences || [],
          facility: store.attributes?.facility || [],
        ...
        serviceCategoryData: store.storeServices || [],
        mapRadiusData: store.nearbyStoresCollection?.items || []
      };

      console.log("%05:Mapped store sections:", mappedData);
        ...
        <div id="store" class="main grid05">
          <div id="section" class="hero col05">
            <div class="col05 array content" id="store-hero">
              ${components.storeHero.render(mappedData.hero)}
            </div>
          </div>
