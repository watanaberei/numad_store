...
const StoreScreen = {
  ...
  after_render: async () => {
    ...
    // Now pass the userData to controlUser.render
    const userControlsHTML = controlUser.render(store, {
  ...
  toggleCheckInStore: async function(storeName, storeId) {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Enable/disable impression buttons based on check-in status
        const impressionButtons = document.querySelectorAll('.impression-button');
        impressionButtons.forEach(button => {
          if (isCheckingOut) {
      ...
      alert(`Failed to ${isCheckingOut ? 'check out from' : 'check in to'} ${storeName}: ${error.message}`);
    }
  },
