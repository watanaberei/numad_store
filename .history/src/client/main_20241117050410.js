...
document.addEventListener('DOMContentLoaded', () => {
 ...
 if (storeHeroContainer && data.store?.hero?.[0]) {
  ...
  if (storeLocationElement) {
    ...
    // const store = data.store?.item?.[0];
    const storeCity = store?.location.city; // Changed from location to Location
    ...
    console.log('debug log: storeLocation02 - Location data check:', {
      ...
      storeCity,
      storeName,
      availableLocations: data.location?.city?.map(loc => loc.city)
    });
    
    // console.log('debug log: storeLocation03 - Current store city:', storeCity);
    console.log('debug log: storeLocation04 - Available locations:', data.location?.city);

    // Find matching location data
    // Find matching location data
    const matchedLocation = data.location?.city?.find(loc => loc.city === storeCity);
    console.log('debug log: storeLocation03 - Location matching:', {
      searchingFor: storeCity,
