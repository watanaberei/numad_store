...
export const getStore = async (limit = 100, skip = 0) => {
  try {
    const query = `
    {
...
export const transformBusinessHours = (hours) => {
  if (!hours) return null;
  
  const daysOfWeek = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  return hours.map(slot => ({
    day: daysOfWeek[slot.day],
    openTime: slot.open.slice(0, 2) + ':' + slot.open.slice(2),
    closeTime: slot.close.slice(0, 2) + ':' + slot.close.slice(2)
  }));
};
...
// Contentful data fetching
export const getStoreData = async (limit = 100, skip = 0) => {
  try {
    const query = `
    {
      appFastFoodHomePage031523Collection(limit: ${limit}, skip: ${skip}) {
