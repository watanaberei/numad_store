...
const normalizeSlug = (slug) => {
  if (!slug) return '';
  // Try both formats
  const withHyphens = slug.toLowerCase().replace(/_/g, '-');
  const withUnderscores = slug.toLowerCase().replace(/-/g, '_');
  return [withHyphens, withUnderscores];
};

// Update store fetching logic
export async function getStore(slug) {
  try {
    const slugs = normalizeSlug(slug);
    let response = null;
    let error = null;

    // Try both slug formats
    for (const slugFormat of slugs) {
      try {
        response = await fetch(`${API_URL}/api/stores/${slugFormat}`, {
          method: 'GET',
          headers: {
        ...
        error = e;
        console.error(`Failed to fetch store with slug ${slugFormat}:`, e.message);
      }
    }

    // If we get here, both formats failed
    throw error || new Error('Failed to fetch store data');
  } catch (error) {
...
// Update store visit endpoint
export const recordStoreVisit = async (slug) => {
  try {
    const slugs = normalizeSlug(slug);
    let response = null;
    let error = null;
    ...
    for (const slugFormat of slugs) {
      try {
        response = await fetch(`${API_URL}/api/stores/visit/${slugFormat}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
        ...
        error = e;
        console.error(`Failed to record visit for slug ${slugFormat}:`, e.message);
      }
    }

    // If we get here, both formats failed
    throw error || new Error('Failed to record store visit');
  } catch (error) {
    console.error('Error recording store visit:', error.message);
    throw error;
  }
};
